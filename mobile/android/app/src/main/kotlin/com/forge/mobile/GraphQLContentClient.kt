...
class GraphQLContentClient(
  ...
  override suspend fun getContent(locale: String, slug: String): MobileContentItem? {
    val response =
      apolloClient
        .query(ExperienceBySlugQuery(slug = slug, locale = com.apollographql.apollo.api.Optional.present(locale)))
        .execute()

    val experience = response.data?.experiences?.firstOrNull() ?: return null

    // Build a summary body from the first CTA or PromoBanner heading
    val body = experience.sections?.mapNotNull { section ->
      when (section) {
    ...
    return MobileContentItem(
      ...
      title = experience.sections?.mapNotNull { section ->
        when (section) {
   ...
   */
  suspend fun listExperiences(locale: String, page: Int = 1, pageSize: Int = 25): List<MobileContentItem> {
    val response =
      apolloClient
        .query(
          ExperiencesQuery(
