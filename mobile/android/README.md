...
| Item | Detail |
|------|--------|
| Schema source | `apps/cms/schema.graphql` (copied to `app/src/main/graphql/schema.graphqls`) |
...
Set endpoint and token in `app/build.gradle.kts` via `buildConfigField`:

```kotlin
buildConfigField("String", "GRAPHQL_ENDPOINT", "\"https://cms.forge.dev/graphql\"")
buildConfigField("String", "GRAPHQL_TOKEN", "\"your-token-here\"")
```
