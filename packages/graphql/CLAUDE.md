...
- **Web queries** → `adminGraphql`
- **Mobile Strapi callsites** → `graphql` until the mobile cutover lands
- **Both in the same file** is only expected in transitional code; prefer a single factory per consumer surface.
...
Anything else is gated by admin's scope-auth plugin and will fail at runtime if queried anonymously.
