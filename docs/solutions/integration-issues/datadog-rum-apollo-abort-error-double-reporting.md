...
- **Filtering at the SDK layer.** Suppressing the native `-999 "cancelled"` errors requires `errorEventMapper`, which `@datadog/mobile-react-native@3.5.2` only exposes as a documented constructor param on `FileBasedConfiguration` — and this app deliberately uses the programmatic `DatadogProviderConfiguration` path for its env-var provisioning gate (see `docs/solutions/integration-issues/datadog-rn-resourceeventmapper-programmatic-config-gap.md`). So the native `"cancelled"` stream remains by design; only the JS-side `"Aborted"` half is fixable in code. The comment at `apps/mobile/src/components/DatadogRum.tsx:86-88` now records this split.
