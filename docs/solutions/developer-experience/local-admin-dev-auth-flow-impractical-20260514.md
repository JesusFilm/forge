...
- [`docs/solutions/runtime-errors/nextjs-alloweddevorigins-hydration-dead-127-0-0-1-20260520.md`](../runtime-errors/nextjs-alloweddevorigins-hydration-dead-127-0-0-1-20260520.md) — **Counterpart on the apps/web side, opposite resolution.** Same dev-origin guard mechanism, same `127.0.0.1` host preference (inherited when a browser tab follows a link from admin at `127.0.0.1:3003` to web at `127.0.0.1:3000`), but `apps/web` has no auth-host proxy to collide with — so `allowedDevOrigins: ["127.0.0.1"]` IS the right fix in `apps/web/next.config.mjs`. Read the two docs together so the resolution doesn't get applied to the wrong app.
