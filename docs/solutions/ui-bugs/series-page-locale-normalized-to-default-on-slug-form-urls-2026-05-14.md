...
2. **Mirror the `WatchPageClient` pattern when adding a new page-route consumer.** That route already separates the bcp47 locale from the slug-form language identifier. Any new branch in `[slug]/[locale]/page.tsx` should follow the same split. This is the load-bearing recurrence-prevention rule — the bug has come back twice now on web precisely because the pattern wasn't carried forward. A **third recurrence** later hit `apps/mobile` through a different mechanism — bcp47 _prefix collision_ actively mis-matching the wrong language rather than validator _rejection_ falling back to a default (see Related). The cross-cutting rule is the same on every surface: never key a user-facing language identity on bcp47; use the slug.
...
- `docs/solutions/best-practices/language-identity-on-slug-not-bcp47-20260605.md` — **third recurrence** of this bug family, on `apps/mobile`. Distinct mechanism: persisting a dub/subtitle choice keyed on a bcp47 _prefix_ actively mis-matched the wrong language (Korean `ko` → Kurmanji Standard `ko-kmr`; English `en` → `en-nai`), where this doc's mechanism was a bcp47 _validator rejecting_ a valid slug. Both reinforce the same rule — key language identity on the slug, never bcp47 — and prove bcp47 can not only reject but actively select the wrong language.
