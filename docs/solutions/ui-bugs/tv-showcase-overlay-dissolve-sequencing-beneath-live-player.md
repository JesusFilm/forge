...
> **Superseded in part (2026-07-21):** the single-player constraint above no longer holds as stated — since `fix/tv-showcase-seamless-hop` (PR #1632) the reel runs TWO long-lived players (the leak trigger is player/view CHURN, not a second fixed instance), and Language Centerpiece hop boundaries crossfade between them. This doc's actual law — a dissolving overlay is only as soft as the layers beneath it, so sequence the covered layers — is UNCHANGED and still governs chapter-card/interstitial seams. See `docs/solutions/ui-bugs/tv-showcase-dual-player-crossfade-dub-hop-blanking.md`.
