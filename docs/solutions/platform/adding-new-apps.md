...
- **CMS access**: always use `@forge/graphql` typed operations or `src/cms/strapiClient.ts` — never REST
...
See `apps/manager/` for a complete example of a Next.js app with external service integrations (Mux, OpenRouter, Cloudflare R2, workflow.dev) added to this monorepo.
