...
- **A client binary cannot hold a secret.** `EXPO_PUBLIC_*` (and `NEXT_PUBLIC_*`) vars are baked into the shipped bundle; "just add a token" trades a correctness bug for an extractable-credential security hole. The qualification (2026-06-12): a **zero-permission passport** (the `CONSUMER_BEARER` class) may ship in the binary because extraction yields rate-limit budget abuse, not data access — and even then only operation-scoped to the search operation, never globally (global attachment pools the whole fleet into one `consumer:<key>` rate-limit bucket). Written against the then-gated `Search`; since admin #1622 the operation is `WatchSearch` and nothing is gated, so the bearer buys bucket identity alone — scoping still matters, because a mismatch now fails silently. A _permission-bearing_ token in a client bundle remains categorically wrong.
