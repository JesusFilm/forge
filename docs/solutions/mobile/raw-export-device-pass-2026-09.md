...
row names its own device, because the two differ — never read a row against the
column header.
...
iOS column below, unless a row names its own device: **iPhone 17 Pro Max
simulator, iOS 26.4**, development build from this branch, Metro from this
worktree, local admin on `:3003`. Rows 11 and 12 name a different simulator.
...
| 11  | Export a multi-episode series. Do episodes save in episode order?           | **iPhone 17 simulator, iOS 26.5, 2026-09-14.** `washi-gospel`, 5 episodes, Highest. The report card named each episode in turn — Can God be Known?, What are Humans?, What is sin?, What is the Cross of Jesus? — which is the grid order, and the count advanced 1 of 5, 2 of 5, 3 of 5. A separate full run over the same series produced 5 photo-library inserts. Transfer ORDER observed; the camera roll's own ordering was not inspected.                                                                                                                                                                                                                        |                                        |
| 12  | Cancel part-way from the running-export control. Do saved episodes remain?  | **iPhone 17 simulator, iOS 26.5, 2026-09-14.** Stopped a 5-episode run after 3. The 3 already saved remained, and the card read "Saved 3 of 5 episodes". This row was BROKEN until PR #2232: the stop did not end the run. See [the run-scoped cancel learning](../logic-errors/series-export-cancel-scoped-to-episode-not-run.md). The pre-fix build was NOT exercised on device — the defect came from code review, and only the fixed build was measured here. **Oracle: count photo-library inserts; read 8 at the stop and still 8 thirty seconds later. Do NOT read the series control — it returns to "Download all" in the broken and the fixed build alike.** |                                        |
...
paths behave. Series transfer order and series cancel are now observed too
(rows 11 and 12, 2026-09-14). Everything about interruption, reuse, size
limits, network transitions, Android, and real hardware is still open.

One result from row 12 generalises past this gate, so it is stated here as
well: an export writes no offline record, so no offline-derived control can
witness one. A verifier who reads the series control learns nothing — it is
idle throughout a run whether the run is healthy or not. Count the
photo-library inserts instead. The reasoning is in
[the run-scoped cancel learning](../logic-errors/series-export-cancel-scoped-to-episode-not-run.md).

Before this gate can be called met: the remaining unfilled rows, both Android
API levels, and the disable-switch table.
