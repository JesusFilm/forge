...
5. **Size the default ceiling above a contract-derived legitimate payload.** Here a `topK=5` retrieval is five short passage chunks plus small citations; a very generous ~50 KB/passage upper bound is ~250 KB. The default is `2_097_152` (2 MiB) — ~8× headroom for this byte-denominated contract — so a valid retrieval is never wrongly rejected as `unavailable`. (The right magnitude comes from the contract, not from a universal low-MB rule: the char-denominated contract in the corollary below legitimately derives 8 MiB.)
   ...
   **Per-field-group corollary — a page-size constant is budgeted against ONE request shape (feat-337, 2026-08-17).** The sizing chain above has a third input beside unit width and item count: **which fields the request asks for**. A listing page size derived for one field set silently under-derives when a second caller reuses it for a request that adds a larger field group — the constant looks shared, but its byte budget was measured against a smaller row. Worked instance: the Langfuse retention sweep's `LANGFUSE_RETENTION_LIST_PAGE_SIZE = 500` was budgeted in its own docblock as "500 **core**-fields rows are ~150 KB" (~300 B/row) against the 256 KiB `LANGFUSE_MAX_RESPONSE_BYTES` cap. feat-337's erasure listing reused it for `fields=core,basic` — the group that adds `userId` plus a dozen metadata fields — and the real measurement (opt-in read smoke, 2026-08-17) came back **63,323 B per 100-row page = ~633 B/row**, so a full 500-row page is ~316 KB: **over the cap**. The failure shape is rule 3's own graceful path turned against a legitimate caller — over-cap → capped reader returns `undefined` → `parse_error` → the erasure run fails identically on every rerun, deterministically, for exactly the **heavy-but-legitimate** subjects erasure most needs to reach. Four rules ride it: **(1) derive a page-size constant per request shape** — a second caller with a different `fields`/projection/include set gets its own constant (`LANGFUSE_ERASURE_LIST_PAGE_SIZE = 100`, `apps/mastra/src/mastra/langfuse-trace-retention.ts`) whose docblock names the field group it was measured against; reusing the sibling's constant is a sizing claim, not a style choice — it asserts the two requests have the same worst-case row, which is false the moment the field sets differ; **(2) size from the LARGEST plausible row, never the measured mean** — row bytes are caller/user-influenced metadata, so the mean is a floor, not a bound; the shipped constant's actual headroom: 100 rows × ~633 B ≈ 63 KB against the 256 KiB cap, ~4× margin, chosen because per-row size is unbounded in practice; **(3) make the opt-in real-credential path measure AND assert** — it must page at the page-size CONSTANT itself (imported, never a local literal) and ASSERT the measured page bytes against the cap with stated headroom, because a log line alone is an observation nobody is obliged to read; `discovery_page_bytes=<n>` stays as the logged evidence, COUNTS ONLY as a binding clause (never a sampled row, body fragment, or subject identifier — the measured store can hold special-category conversation content), and the smoke's limits are named so it is not read as a standing guarantee (opt-in, discovery rows are an unfiltered proxy for the filtered listing, and the read deliberately sits outside the production capped reader); and **(4) a measurement that falsifies the row-width assumption behind a SIBLING constant obliges re-measuring the sibling, not just splitting the constant** — applied same-day: the sweep's `fields=core` row measured ~356 B (2026-08-17, read-only probe), so a full 500-row page projects to ~178 KB — safe under the cap at ~1.5× headroom, and the retention smoke now measures-and-asserts it per rule 3 (the "~300 B/row" docblock estimate was close but was still an estimate until measured). This corollary was found at review time as an anchor-50 "may exceed" hunch and became an empirical fact only because a smoke run measured a real page — the measurement corollary above, applied one level up.
