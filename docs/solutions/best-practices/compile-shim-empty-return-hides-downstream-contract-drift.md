...
**Field selection.** #1622 deleted the assertion that the search document selects `label` and `childCount`. Those two fields drive series routing (`isSeriesSearchResult`), so when the new document stopped selecting them, every series result would have degraded to the `/watch` hop with nothing failing. Restored at `apps/tv/src/lib/queries.test.ts:44`.
