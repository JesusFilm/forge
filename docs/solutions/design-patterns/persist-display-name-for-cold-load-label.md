...
- `docs/solutions/design-patterns/mobile-subtitle-preference-reconciliation-contract.md` — the broader preference-reconciliation contract this cold-load label fix is one rule of: the `null` (not loaded → optimistic cached name) vs `[]` (loaded-empty → "Off") distinction this doc introduced, plus reconcile-against-real-tracks and toggle-vs-select persistence.
