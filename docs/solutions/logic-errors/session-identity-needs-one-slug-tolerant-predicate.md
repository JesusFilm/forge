...
- An expand restart had one earlier, different cause, fixed on 2026-08-18: the watch session defaulted its dub index to 0, so one render published the first dub's stream, which read as a dub switch and defeated the adoption pin. That fix made the index null until the default resolves. The restart symptom has therefore had two distinct causes in this component; check both before assuming either.
