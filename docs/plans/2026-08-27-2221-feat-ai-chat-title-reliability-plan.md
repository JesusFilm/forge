...
> **Superseded (2026-09-08, feat-464):** The backend pair in KTD4 was
> simplified: title repair now has one backend check,
> `canAiChatDataPersist()`, which means the shared `MASTRA_STORAGE_BACKEND` is
> Postgres. The per-surface resolver and memory kill switch no longer exist.
> `SEEKER_ROUTE_ENABLED` still gates this workflow and the custom Forge
> ai-chat routes, but it does not disable native `/api/agents/seekerAgent`;
> that native surface remains contained by the Mastra gateway and Railway
> network boundary.
...
- **Approach:**
  1. `apps/mastra/CLAUDE.md`: add the `AI_CHAT_TITLE_REPAIR_ENABLED` env-table row; extend the `AI_GATEWAY_SEEKER_ENABLED` row's coupling note (it now also selects the titling chain); update the "Titles (KTD12)" bullet in the history-read section; add a short sweep subsection (schedule, gates including the two-lever retraction order — `SEEKER_ROUTE_ENABLED=false` darkens the custom Forge routes and skips the sweep, `AI_CHAT_TITLE_REPAIR_ENABLED=false` is the fine-grained lever — and pause/resume inheriting the Instagram affordance). Per the 2026-09-08 scope clarification above, the route flag does not disable native `/api/agents/seekerAgent`.
