...
- KTD1. **The production-host refusal is raised at `apps/mobile/src/env.ts` module evaluation, not inside `getGraphQLUrl()`.** The three callers are `apps/mobile/src/lib/apolloClient.ts:225`, `apps/mobile/src/lib/datadog.ts:98`, and `apps/mobile/src/hooks/useVideoThumbnails.ts:82`. They handle a synchronous throw three different ways: the thumbnails caller swallows it in a bare catch at `:110`; the Datadog caller only runs when credentials are provisioned; the Apollo caller throws from `RootLayout`'s own body, outside the ErrorBoundary rendered in its returned tree, and other `getApolloClient()` call sites catch it. So a refusal raised in the shared accessor is inconsistent at best and invisible at worst. Module scope in `env.ts` runs before any render and before all three, and `apps/mobile/app/_layout.tsx` already wraps that require in a try/catch that renders a full-screen selectable Startup Error panel showing the thrown message verbatim. R2 therefore needs no new UI.
  ...
  > **Correction — 2026-08-07, amended 2026-08-11 (post-implementation).** The final sentence overstates a guarantee, though its conclusion holds. `_layout.tsx`'s try/catch catches this throw **only when its guarded require is the first evaluation path into `env.ts`** — a property of the import graph, not of the guard, and one that ordinary feature work changes. Both surfaces have now been observed on this app: on 2026-08-07 the refusal escaped to the RN dev error overlay with the stack `env.ts -> config.ts -> apolloClient.ts -> useWatchHome.ts` (a screen's static import chain reaching `env.ts` outside the guard); on 2026-08-11, after main's #1876 changed `_layout.tsx`'s require block, the guard caught it and the Startup Error panel rendered. R2 is satisfied either way — both surfaces show the message verbatim and selectable — and the refusal is `__DEV__`-gated, so no release bundle can reach it. The enforcement point (env module scope rather than `getGraphQLUrl()`) is unaffected; what is corrected is treating the panel as a guaranteed surface. Full mechanism: `docs/solutions/best-practices/expo-router-require-guard-containment-is-order-dependent.md`.
...
> **Correction — 2026-08-07, amended 2026-08-11 (post-implementation).** The `Layout` lane above is idealized. Route modules are evaluated through expo-router's own graph, so a chain like `useWatchHome.ts -> apolloClient.ts -> config.ts` can reach `Env` outside `_layout.tsx`'s guarded require — in which case `moduleError` is never set and the dev error overlay displays the refusal instead. Which path reaches `env.ts` first decides it, and both outcomes have been observed. See the correction under KTD1. The `Env`-step placement itself is unchanged and still correct.
