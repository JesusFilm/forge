...
- KTD11. **Pure-extraction test discipline.** `apps/mobile` has no component-render test infrastructure, so every behavioral decision (thresholds, queue, dedupe, account binding, fail-open, header scoping) lives in pure `src/lib` modules with unit tests; components stay thin. No new test dependencies.
  ...
  > **Superseded, 2026-08-18 (feat-367).** The premise no longer holds: `apps/mobile` now has a component-render harness, and it still needed no new test dependency — jest-expo's own transitive `react-test-renderer`, reached through a `react` re-point. See `apps/mobile/CLAUDE.md`, section "Component render tests". Pure extraction remains the default for decision logic; it is no longer the ONLY option for component behaviour. Later plans that cite this KTD11 inherit this note.
