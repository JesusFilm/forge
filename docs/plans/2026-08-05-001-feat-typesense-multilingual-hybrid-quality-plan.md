...
- R3. The lexical serving projection covers the repository's fixed 30-locale Search Eval corpus, including English, Mandarin/Chinese, Thai, Arabic, Hindi, Japanese, Korean, and regional variants. Non-English fields use Typesense locale-aware tokenization where a supported two-letter locale exists; unknown/long-tail locales retain a measured generic fallback.
- R4. A query such as `Jesus` can retrieve the relevant JESUS product family in English, and equivalent translated queries can retrieve the appropriate canonically related, playable result in Mandarin, Thai, and other supported languages without requiring English terms in the query.
...
- KTD4. Make multilingual lexical indexing schema-explicit: one optional title field and one optional metadata field per supported base tokenizer locale, plus generic fallback fields for long-tail content. Resolve the query's canonical BCP-47/base locale to a bounded field list, and never search every language field on every request. Governs R1-R4, R13.
...
- The fixed 30-locale Search Eval corpus is the initial locale-aware tokenization contract. It is grounded in corpus depth and includes the scripts explicitly requested; new locales can be added through the same schema/versioning path after measured demand.
- Base-language tokenization is suitable for regional BCP-47 variants for this release, while the document retains exact locale codes for display and filtering. Chinese variants use `zh`; Thai uses `th`; generic fallback preserves long-tail recall.
...
- **Approach:** Add a versioned `watch_search_lexical` alias. Project one document per viewer-visible physical video with canonical ID, exact locale codes, locale-specific title/metadata fields for supported base tokenizers, generic fallback text, and only the facets needed for grouping/filtering. Keep display/card data in the existing catalog collection. Build lexical, catalog, and availability as one checked metadata generation while reusing transcripts. Report indexed string bytes plus the 2x-3x Typesense keyword estimate.
...
- **Test scenarios:**
  1. Project English, Chinese, Thai, Arabic, regional, and unknown locale rows into the expected locale-specific/fallback fields without duplicating a value across searchable fields.
...
- **Test scenarios:**
  ...
  2. Chinese and Thai queries select their locale-aware fields; an unsupported locale searches only its bounded fallback fields.
...
- Locale-aware title/metadata fields cover the fixed 30-locale evaluation corpus, including English, Chinese/Mandarin, and Thai, with bounded generic fallback.
