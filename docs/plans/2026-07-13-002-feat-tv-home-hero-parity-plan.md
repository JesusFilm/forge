...
> **Amendment 2 — FINAL (2026-07-13, label gate / "Option 1"; supersedes Amendment 1 below):** Emitting the parent for _every_ source still diverged from web, because web's hero gates on a build-time **`hls` stream** — which drops `COLLECTION`/`SERIES` containers (their parents have no `hls`) while keeping the parent feature films and short films. TV/mobile use a lean fetch (no `variants`/`hls`) so they cannot replicate that gate exactly. The owner chose **Option 1**: approximate it with a **wire-`label` gate** — `eligibleCardsForSource` (TV) and `eligibleSlidesForSource` (mobile) now `return []` for `label === "COLLECTION" | "SERIES"`, keeping `FEATURE_FILM`/`SHORT_FILM` (feature films kept even with chapter-children, e.g. JESUS). Result = web's set (JESUS, Gospel of John + Christmas shorts) **plus a couple of extra feature films** web drops only for missing `hls` (e.g. Magdalena) — an accepted over-inclusion. Emptying the collection pools also lets the `shortFilms` pool become reachable in the 7-slot queue, so the Christmas shorts now surface on TV/mobile as on web. Web untouched. Golden regenerated from mobile's real label-gated output; sim-verified on tvOS.

> **Amendment 1 (2026-07-13, after live web comparison — superseded by Amendment 2 above):** The premise "web and mobile show the same hero" is FALSE. Web renders each playlist source as its **parent film/collection** (its child expansion is dead — the web GraphQL fragment omits child `variants`/`hls`, so children always fail web's hls gate and it falls back to the parent). Mobile/TV gate on `poster+slug`, which children have, so they **expand to child episodes** — a pre-existing web↔mobile divergence. The user chose to **converge all three apps on web's parent behavior**: TV **and mobile** now emit the PARENT film/collection per source, not child episodes. This is a TV **and** mobile change (`eligibleSlidesForSource` / `eligibleCardsForSource` in both apps). Web is untouched (already correct). TV still excludes Mux inserts; mobile keeps them. The cross-app golden was regenerated from mobile's new parent output.
