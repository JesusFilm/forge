...
| **Synthetic watch blocks**     | The 6 `kind: "HeroPlayer"                                                                                                                                                                                                                       | "SiblingCarousel" | "WatchBody" | "StudyQuestions" | "BibleQuotes" | "Share"`discriminants the web watch route invents at resolve time. NOT Strapi`\_\_typename`values — they exist purely so the watch renderer can dispatch watch-only components alongside Strapi-typed Experience blocks. Defined in`apps/web/src/lib/content.ts` `WatchBlock` union. |
...
| **PUBLIC tier**             | Admin's anonymous-request authorization scope. Today exposes 4 queries: `experienceBySlug`, `searchExperiences`, `hybridSearch`, `sceneRecommendations` (per origin parent plan Unit 2). Anything else requires authentication. The inventory flags every consumer operation against this list. |
...
| Term                     | Meaning                                                                                                                                                                                                                                                                        |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **gql.tada `graphql()`** | The typed factory exported from `@forge/graphql`. Operations defined in consumer apps as `` graphql`query { ... }` `` get their full TS types inferred from the committed Strapi introspection. This inventory's "every operation" claim = "every callsite of `graphql(...)`". |
...
| `apps/admin/src/domain/blocks.ts`                                      | Admin's Zod block union — the 17 top-level `t` literals plus 3 nested-scope unions (Block / SectionContent / ContainerContent). The mapping target.                                                |
| `apps/admin/src/auth/permissions.ts`                                   | Admin's PUBLIC-tier definition. The 4 anonymous-readable queries the inventory cross-checks against.                                                                                               |
...
- R7. The verification command from the parent plan (`rg "graphql\(" apps/web/src apps/mobile/src apps/tv/src`) produces output that the inventory document accounts for line-by-line — no callsite present in the rg output is absent from the inventory, and no inventory entry references a callsite absent from rg output. (origin Unit 1 Verification)
...
- `apps/admin/src/auth/permissions.ts` — admin's PUBLIC-tier authorization. The cross-check target for U5's PUBLIC classification pass
...
- **PUBLIC classification is binary + "Unit 2 must widen".** Each operation gets one of three labels: (a) **PUBLIC-current** = already in admin's 4-query PUBLIC tier per `apps/admin/src/auth/permissions.ts`; (b) **PUBLIC-eligible-needs-widening** = should be PUBLIC but admin doesn't expose it yet (Unit 2 work); (c) **MUST-stay-authenticated** = preview/draft reads, never PUBLIC. Rationale: this exact tripartite is what Unit 2's planning needs as input — anything fuzzier creates a per-operation negotiation that Unit 2 will have to redo.
...
## apps/web Operations
  ...
  ### web:GET_VIDEO_BY_SLUG (recommendations.ts)
...
- Read: `apps/web/src/lib/recommendations.ts` (1 operation: `GET_VIDEO_BY_SLUG`)
- Read: `apps/web/src/lib/client.ts` (Apollo client + cache config to source the cache-behavior column)

**Approach:**

- Run `rg "graphql\(" apps/web/src` first; treat the output as the work list. Every line becomes a `### web:{ConstantName}` subsection in the inventory
...
- Same rg-driven enumeration approach as U1, applied to `apps/mobile/src`
...
- Same rg-driven enumeration as U2
...
- Read: `apps/admin/src/domain/blocks.ts` (17 `t` literals + 3 nested-scope unions)
...
- For each typename, look up the corresponding admin `t` literal in `apps/admin/src/domain/blocks.ts`. The 17 admin top-level literals are: `adventCountdown`, `bibleQuotesCarousel`, `card`, `container`, `cta`, `easterDates`, `infoBlocks`, `mediaCollection`, `navigationCarousel`, `promoBanner`, `quizButton`, `relatedQuestions`, `section`, `text`, `video`, `videoCarousel`, `videoHero` (plus `videoRecommendations` which is admin-side-only — no Strapi counterpart, flag this asymmetry)
...
- Read: `apps/admin/src/auth/permissions.ts` (current PUBLIC tier definition — 4 queries per origin parent plan: `experienceBySlug`, `searchExperiences`, `hybridSearch`, `sceneRecommendations`)
...
- For each inventoried operation across all three apps, classify into one of three buckets:
  1. **PUBLIC-current**: maps to one of admin's 4 already-PUBLIC queries (`experienceBySlug` ↔ web's `GET_EXPERIENCE` and `GET_WATCH_EXPERIENCE` ; `searchExperiences` / `hybridSearch` ↔ `*:SEMANTIC_SEARCH`; `sceneRecommendations` ↔ `web:GET_VIDEO_BY_SLUG` (for recommendations)). Verify these mappings against `permissions.ts` — do not assume from name alone
  2. **PUBLIC-eligible-needs-widening**: should be PUBLIC (no preview-mode signal, no auth-required header) but admin doesn't yet expose a corresponding PUBLIC query. These rows feed Unit 2's planning. Likely candidates: `web:GET_ROUTE_VIDEO`, `web:GET_WATCH_SETTINGS`, `mobile:LIST_EXPERIENCES`, `tv:LIST_EXPERIENCES`, `web:getWatchVideoOperation`, `web:getWatchVideoBySlugOperation`. Confirm during writing
  3. **MUST-stay-authenticated**: any operation that reads draft/unpublished content (any preview-mode signal) or uses an admin-only Apollo client. If no consumer operation has preview-mode today, this list is empty — record that as a finding, not a defect
...
- Verification pass: run `rg "graphql\(" apps/web/src apps/mobile/src apps/tv/src` and walk every output line against the inventory. Record the rg output verbatim in the `## Verification Log` section, then record a per-line pass/fail (every line must have exactly one matching inventory entry; the inventory must contain no orphan entries)
...
| **Preview-mode detection is heuristic**: U5's "scan call sites for preview signal" pass may miss a non-obvious draft-read pattern (e.g., a non-standard header injected by a Next.js middleware), silently classifying a MUST-stay-authenticated operation as PUBLIC-eligible | U5 explicitly examines each operation's full call site (Apollo client config, headers, route parameters) — not just the GraphQL document. If the heuristic fails to find any preview-mode signal, the inventory records "no preview-mode operation found in current consumers" as the explicit finding rather than implying every operation is PUBLIC-eligible. Unit 5's web vertical-slice planning will re-verify before any operation is actually flipped to PUBLIC in admin |
...
- **Related code (read-only):** `apps/web/src/lib/content.ts`, `apps/web/src/lib/fragments/` (17 files), `apps/web/src/lib/{search,demo-search,recommendations}.ts`, `apps/mobile/src/lib/queries.ts`, `apps/mobile/src/lib/normalizer.ts`, `apps/tv/src/lib/queries.ts`, `apps/tv/src/lib/normalizer.ts`, `apps/admin/src/domain/blocks.ts`, `apps/admin/src/auth/permissions.ts`
