...
- **Deferred (operational, non-blocking for this unit):** Langfuse hosting posture and ownership — Langfuse Cloud (which region: EU vs US, since keys and base URLs are region-bound) vs self-hosted, and who provisions the per-environment projects, key pairs, and the seeded smoke prompt. Does not block U1–U5 or the mocked proof; must be answered before the U4 smoke can ever run and before the follow-up integration starts. The helper is deliberately posture-agnostic (`LANGFUSE_BASE_URL` has no default) so either answer plugs in without code change.
