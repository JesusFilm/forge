...
- KD27. **On a short screen, a window in a bottom corner moves the verse up.** The centered verse area (KD9) is the same height above and below the screen center. On the iPhone SE Bible tab, a bottom-corner window sits at that center, so the area shrinks to nothing. When the centered area is under 160 points, the verse uses the free space between the top bar and the window. Tall phones and iPads keep the centered verse. (session-settled: user-directed on 2026-09-25 — chosen over top corners only on short screens, and over shipping with the verse hidden: the window stays in any corner, and nothing covers the verse.) Governs R7, R10.
