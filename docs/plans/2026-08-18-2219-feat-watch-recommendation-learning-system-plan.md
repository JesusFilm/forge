...
- **Objective:** Establish the complete product contract for a production Watch recommendation system that begins with a semantic baseline and evaluates every additional signal through observable vertical slices.
...
- **Product Contract preservation:** Restructured with no scope change. R1-R46 remain unchanged; the Planning Contract resolves the former planning questions and corrects implementation sequencing.
...
Forge will launch semantic recommendations below the Watch player, measure their full journey, and add every other signal through an evidence-gated signal ladder.
...
- **Use a semantic spine with an evidence-gated signal ladder.** (session-settled: user-directed — chosen over foundation-first delivery and a bundled recommender: semantic-only creates a useful, attributable production baseline.) Governs R1, R25, R35.
...
- **Privacy and scale graduation**
  - Shares every stage through retention, erasure, access control, reliability, cost, and the final infrastructure review in Slice 28.
...
- F4. **Candidate generator competes with semantic-only**
...
- AE7. **Covers R25-R28, R35-R37.** Given profile and co-watch generators are in shadow, when Watch serves semantic-only, then Admin shows their counterfactual candidates and reasons without allowing them to change the live slate.
...
These product questions do not block execution. KTD1-KTD21 and the Verification Contract resolve their implementation mechanism while keeping configurable policy values visible and versioned.
...
- KTD6. **Playback is an append-only episode state machine finalized by a durable workflow.** A valid item token opens a narrowly scoped episode token bound to request, item, session, media, actor class, and bounded late-event horizon. Immutable facts preserve attempt, start, foreground-playing intervals, pause source, seeks, skips, replay, re-entry, route exit, media/language changes, QoE, error, end, timeout, and late evidence. Active time unions bounded intervals so replay cannot inflate it. Each outcome revision cites one classifier version, exact input watermark and digest, supersedes rather than mutates prior output, and publishes with generation fencing. The first tracer retains the legacy `30 seconds OR 25%` position rule as `legacy-position-v0` only for comparison. The next slice introduces `active-watch-proxy-v1` and a versioned continuous quality projection. Neither is named satisfaction or a universal meaningful-watch truth. Governs R10-R14, R21, R26, R32, R36.
- KTD7. **Recommendation identity is separate from Mux and legacy progress identity.** Use an ephemeral session identifier immediately. After the applicable personalization choice or consent, issue a scoped `Secure`, `HttpOnly`, `SameSite` pseudonym and rotate it on consent changes, authentication, merge, or suspected compromise. Token possession can support same-origin personalization and reset/withdraw/delete, but detailed export, raw inspection, and account merge require authenticated ownership and step-up verification. Store profile state server-side; use an idempotent canonical-identity and alias ledger rather than copying evidence during merge. A withdrawal/delete transition synchronously revokes tokens, increments a privacy generation, and fences stale projection publication before asynchronous erasure proceeds. Do not reuse the cross-visit `viewer-id` or anonymous watch-progress storage as profile consent. Governs R16-R18, R22, R41-R46.
...
- KTD14. **Privacy and deletion are ticket-local acceptance gates.** Every data-bearing unit declares purpose, identity class, retention, access, aggregate boundary, cascade/tombstone behavior, and deletion propagation deadline. Raw retention covers the maximum lateness, finalization, and rebuild window; after raw purge, only privacy-reviewed minimal contribution state may survive for exact correction and erasure. Withdrawal/deletion installs an immediate serving/evaluation tombstone, revokes tokens, invalidates viewer caches and assignments, fences stale workers, rebuilds affected projections, and prevents new training inclusion. Restore procedures replay tombstones before traffic. Permitted audit facts are non-relinkable and never reactivate influence. Governs R7, R16-R22, R40-R46.
...
- KTD21. **One immutable strategy manifest pins the complete serving decision.** A manifest records generator, eligibility, feature-set, ranker, composer, outcome-policy, and fallback version digests. Each request pins one manifest plus published projection watermarks from request creation through evidence attribution. Promotion changes only a compare-and-swap active pointer; serving never reads raw evidence, mutable strategy parts, or half-built projections. Governs R2-R3, R14, R24-R40, R41-R46.
...
flowchart TB
  ...
  WF --> PR["Versioned outcomes, profiles, co-watch, evaluations"]
  PR --> CG
  PR --> DR
  PG --> AD
  PR --> AD
...
The first live strategy is semantic-only. The common candidate seam initially has one source so its latency, fallback, provenance, and explanation behavior are proven before other generators arrive. Shadow generators reuse the same context and policy but write only counterfactual projections.

### Recommendation-Owned Data Boundaries

| Boundary               | Initial records                                                                                                                                                                | Authority and lifecycle                                                                                                                               |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| Delivery               | `RecommendationRequest`, `RecommendationServedItem`, `RecommendationImpression`, `RecommendationSelection`                                                                     | Server-created request/item identity; client visibility and selection accepted only with signed attribution and idempotency                           |
...
| Learning               | `RecommendationCandidateRun`, `RecommendationCandidateNomination`, `RecommendationProjectionVersion`, `RecommendationProjectionContribution`, `RecommendationCoWatchEdge`      | Rebuildable generations cite exact input windows, policy versions, contribution units, and privacy generation; raw event queries are forbidden online |
...
6. Add each candidate generator independently in shadow. Create a separate exposure ticket only after its terminal shadow decision says it is eligible.
7. Add advanced slate policy, learned ranking, and bounded exploration after their evidence prerequisites exist.
8. Run the formal scale review after observing the complete service. The review may retain Postgres and pgvector.
...
| U29  | feat-396       | Privacy, capacity, and infrastructure graduation review | `apps/admin/src/services/recommendations/`, `docs/operations/`                                                                   | U1-U28                                    |
...
- **Requirements:** R1-R15, R25, R41-R46; F1; AE1-AE4, AE12; KTD1-KTD6, KTD12-KTD16.
...
- **Requirements:** R10-R14, R21, R26, R32, R36, R41-R46; F2; AE4; KTD2, KTD6, KTD13-KTD16.
- **Dependencies:** U1.
...
- **Approach:** Exchange a valid served-item token for the episode-scoped token, record immutable episode facts, union foreground-playing intervals, and finalize through a fenced idempotent workflow. Outcome revisions cite exact input watermarks/digests and supersede rather than mutate. Compare `legacy-position-v0` with `active-watch-proxy-v1` by duration cohort before the new proxy becomes eligible downstream.
...
- **Requirements:** R15-R18, R22, R28, R30-R31, R35-R36, R41-R46; F3; AE5, AE8; KTD7, KTD12, KTD14, KTD16, KTD18.
...
- **Approach:** Treat surface-derived purpose, optional explicit purpose, long-term interests, session intent, and negative evidence as separate inputs. Explain the immediate effect, offer undo/reset, and expire transient purpose without silently rewriting durable taste.
...
- **Files:** Profile-interest projection workflow, profile candidate generator, context builder, Admin interest and candidate views.
...
- **Requirements:** R13-R14, R19-R20, R23-R28, R30-R31, R35-R37, R41-R46; F4; AE7-AE8; KTD6-KTD7, KTD9-KTD10, KTD13-KTD15, KTD18.
...
- **Requirements:** R14, R19-R20, R24, R30-R32, R35-R40, R41-R46; F2, F4-F5; AE9-AE10, AE12; KTD9-KTD16, KTD18.
...
- **Approach:** Begin with logistic regression or gradient-boosted trees using one versioned feature definition shared by training and serving and separate targets combined only through the explicit surface policy. Build point-in-time train/validation/test snapshots with exact feature, policy, code, eligibility, privacy cutoff, and U28 propensity lineage. Deletion prevents future snapshot inclusion; whether a material deletion revokes an already published model is an explicit privacy-policy field in its manifest. Publish model artifacts atomically, calibrate each outcome, and keep the deterministic ranker as control and fallback. A controlled-exposure ticket is created only if shadow evidence passes.
...
- **Requirements:** R7, R14, R16-R22, R35-R46; F2-F5; AE5-AE6, AE10, AE12; KTD12-KTD16.
- **Dependencies:** U1-U28. Privacy obligations remain acceptance gates on every prior unit. Formal lifecycle/capacity checkpoints also run after U2, U10, and U18; this unit is the final cross-system decision, not the first drill or the only time an observed bottleneck may be addressed.
...
U1 records p50/p95 end-to-end recommendation latency, response bytes, database query time, and player startup timing before enabling the production block. Later units may not regress player startup or exceed the unit's declared serving-budget allocation without an explicit decision. The service records bounded candidate counts, ingestion rate, workflow backlog age, database growth, purge duration, and fallback use so U29 can identify the actual constraint.

## Definition of Done

The plan is delivered when:

- U1-U29 satisfy their requirements and verification, or a roadmap ticket records a product-approved scope change.
