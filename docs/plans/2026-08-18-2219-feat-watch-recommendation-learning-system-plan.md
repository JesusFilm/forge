...
- **Open blockers:** None block implementation planning. Statistical methods, metric thresholds, retention periods and rollout percentages are deferred to planning or later calibration without changing the product contract.
- **Product Contract preservation:** R16-R18 use the September 10 default-enabled personalization decision. R47-R50 append source-neutral preference evidence, explicit title feedback, learned representation, and page-orchestration contracts without weakening the settled semantic, privacy, evidence, or fallback boundaries.
...
- **Enable anonymous personalization by default.** (user decision, September 10, 2026: configured session and durable-profile personalization need no consent prerequisite; explicit disable, reset, and deletion remain effective.) Governs R16-R18.
...
- A1. **Anonymous viewer:** Uses Watch without an account and receives a default-enabled durable first-party interest profile, with explicit disable/reset/delete controls.
- A2. **Signed-in viewer:** Can carry eligible preferences, history, saved work, and recommendation controls across devices.
...
- R16. Session context may personalize the current visit immediately. A durable anonymous profile starts by default when personalization is configured and uses a first-party opaque identifier rather than storing profile contents in the cookie.
- R17. Anonymous profiles must support reset, deletion, disabling personalization, expiry, and an explicit optional merge into a signed-in profile without making the anonymous identifier a general account credential.
...
- R47. Every finalized Watch outcome eligible under integrity checks and explicit personalization settings must be eligible for preference projection under one source-neutral policy regardless of recommendation, search, direct, shared-link, acquisition, or editorial discovery; source remains independent provenance and may be a versioned feature, never the eligibility gate.
...
|    10 | Default-enabled anonymous continuity      | Session and cross-visit personalization work by default; viewers can disable personalization or reset the anonymous profile.           | Session versus durable-profile coverage, personalization-setting transitions, reset, deletion, expiry, and merge health.                            | Slices 1-2                                           |
...
|    16 | Multi-interest profile candidates         | Anonymous session interests and eligible durable interests produce profile candidates in shadow and controlled exposure.               | Interest-cluster evidence, candidate coverage, novelty, overlap, personalization-setting cohort, counterfactual slate, and experiment result.       | Slices 10-15                                         |
...
- **Identity, reported value, and defensibility**
  - Depends on the evidence chain and enables Slices 10-13 through default-enabled anonymous identity, intent, integrity, and surveys.
- **Controlled learning**
...
- F3. **Anonymous viewer controls profile persistence**
  - **Trigger:** A1 begins a session, makes the applicable personalization choice, resets personalization, disables personalization, signs up, or requests deletion.
  - **Actors:** A1, A2, A5, A6
...
- F8. **Eligible sequence becomes a learned retrieval projection**
  ...
  - **Outcome:** Forge gains a reusable learned retrieval signal without reading raw history online or making the model authoritative for evidence, identity, personalization settings, or item truth.
...
- AE13. **Covers R47.** Given two viewers with personalization enabled and eligible evidence produce equivalent qualified outcomes after recommendation and direct/shared discovery, when preference projection runs, then both outcomes pass the same eligibility policy while their distinct source provenance remains inspectable.
...
- Equivalent qualified outcomes can influence preferences regardless of discovery source, and explicit title actions retain distinct reversible meanings.
...
- A learned profile/item representation is a future projection and candidate source, not permission to replace semantic fallback, raw evidence, personalization settings and integrity policy, independently inspectable co-watch truth, or the multi-outcome re-ranker.
...
- Apply the configured personalization default and honor explicit disable/reset/delete controls as specified in `docs/analytics-and-recommendation-policy.md`. Consent approval is not a release prerequisite.
...
- Which retention periods, expiry behavior, reset/merge/export flows, and erasure deadlines apply to each data class?
...
- KTD7. **Recommendation identity is separate from Mux and legacy progress identity.** Use an ephemeral session identifier immediately. When personalization is configured and the viewer has not disabled it, issue a host-only `Secure`, `HttpOnly`, `SameSite` pseudonym and rotate it on personalization-setting changes, authentication, merge, or suspected compromise. Persist only a one-way digest of the raw cookie value and exclude the raw value from logs, traces, telemetry, and URLs. Token possession can support same-origin personalization and reset/withdraw/delete, but detailed export, raw inspection, and account merge require authenticated ownership and step-up verification. Store profile state server-side; use an idempotent canonical-identity and alias ledger rather than copying evidence during merge. A withdrawal/delete transition synchronously revokes tokens, increments a privacy generation, and fences stale projection publication before asynchronous erasure proceeds. Keep the cross-visit `viewer-id` and anonymous watch-progress storage separate from recommendation identity. Governs R16-R18, R22, R41-R46.
...
- KTD22. **The first live profile strategy is one hybrid pipeline, not a profile-only lane.** The configured personalized request retrieves semantic and bounded published profile nominations through the common candidate seam, unions their provenance, applies eligibility/ranking/composition once, and keeps semantic-only as behavior when personalization is explicitly disabled, experiment control, and last-known-good fallback. Selection may update bounded session intent; durable interest influence requires an integrity-eligible finalized outcome. The complete request stays within the existing 1.5-second deadline. Governs R1-R3, R14, R16-R18, R23-R25, R28, R30-R31, R35-R46.
...
| Identity and trust     | `RecommendationSession`, `RecommendationProfile`, `RecommendationProfileInterest`, `RecommendationConsentTransition`, `RecommendationIntegrityDecision`                                                          | Session context is ephemeral; durable profile state honors explicit personalization settings; actor and eligibility classes are independently auditable                              |
...
3. Establish machine separation, default-enabled anonymous identity, integrity eligibility, and reported-value calibration.
...
6. Expose the approved multi-interest profile source only through the governed hybrid pipeline while semantic remains behavior when personalization is explicitly disabled, control, and fallback.
...
- **Schemas and retention:** KTD1, KTD2, KTD13, and KTD14 define the boundary. Each data-bearing unit records an explicit retention value and cannot ship raw capture without purge health.
...
- **Personalization controls and erasure:** KTD7 and KTD14 preserve default-enabled personalization, explicit disable/reset/delete controls, and deletion propagation.
...
| U10  | feat-378       | Default-enabled anonymous continuity                    | `apps/web/src/lib/`, `apps/admin/src/services/recommendations/`, `apps/admin/prisma/schema.prisma`                               | U1, U2, U12                               |
...
| U30  | feat-447       | Live hybrid personalization rollout                     | `apps/admin/src/services/recommendations/`, `apps/web/src/components/recommendations/`                                           | U17-U19                                   |
...
### U10. Default-enabled anonymous continuity

- **Goal:** Use session context and controllable cross-visit personalization by default when configured, honoring a saved choice to disable personalization.
- **Requirements:** R16-R18, R22, R41-R46; F3; AE5; KTD7, KTD12, KTD14, KTD16.
- **Dependencies:** U1, U2, U12.
- **Files:** Recommendation session/profile models and services, Watch personalization controls, personalization-setting and erasure workflows, Admin privacy health views.
- **Approach:** Create a session-scoped recommendation identity by default, issue the protected pseudonym by default when configured unless the viewer has disabled personalization, and implement reset/withdraw/delete/expire plus authenticated export and explicit merge. Withdrawal/delete atomically tombstones the current privacy generation, revokes the token, invalidates assignments/caches, and fences stale workers before asynchronous erasure. Merge records aliases without copying evidence. Add a deletion drill before profile-derived candidates can ship.
- **Test scenarios:** Default-enabled personalization without a consent interaction; explicit disable and later re-enable; fixation/substitution/stolen-cookie attempts; CSRF; reset creates a new generation; concurrent merge remains idempotent; cross-account merge fails; a paused worker cannot publish after deletion; restore replays the tombstone before traffic; permitted audit facts cannot relink the viewer.
...
- **Test scenarios:** Dismissal, no response, response after short mission-valued watch, negative response, locale fallback, disabling personalization, machine exclusion, keyboard/screen-reader use, and cohort suppression.
...
- **Files:** Profile-interest projection workflow, profile candidate generator, context builder, Admin interest and candidate views; `docs/roadmap/content-discovery/feat-386-multi-interest-profile-candidates.md`; focused projection, generator, personalization controls, erasure, and Admin candidate tests beside the owning modules.
...
- **Test scenarios:** Two unrelated interests remain distinct; session intent temporarily dominates without rewriting long-term interests; viewer who disables personalization receives contextual recommendations without profile use; reset/delete removes influence; sparse profile falls back to semantic.
- **Verification:** Projection and generator tests, personalization-control/erasure tests, interest-level provenance, and Admin cluster stability, coverage, novelty, overlap, cohort quality, and terminal decision.

<!-- U30 is sequenced here because it promotes the U19 source through U17-U18 before later generator work. -->

### U30. Live hybrid personalization rollout
...
- **Files:** Recommendation profile/candidate/experiment/admin-ops services and workflows; GraphQL contracts; Watch session, recommendation, playback, and personalization controls; Admin request trace; `docs/roadmap/content-discovery/feat-447-live-anonymous-profile-personalization-pilot.md`.
...
- **Test scenarios:** Reformulated query replaces short-lived intent, machine query excluded, no-result query does not become positive interest, intent expires, disabling personalization leaves session behavior only, and semantic control remains live.
- **Verification:** Expiry and separation tests, generator coverage/latency tests, payload-minimization checks, and Admin intent evidence, overlap, novelty, expiry, and terminal decision.
...
- **Approach:** Run access, personalization-disable, erasure, projection rebuild, workflow recovery, cache rollback, full-corpus semantic retrieval, ingestion/load, storage-growth, and complete-service latency exercises. Attribute any breach to a specific bottleneck and document retain/optimize/split decisions. `No infrastructure change` is a valid outcome.
...
- Disabling personalization and deletion propagate through profiles, co-watch contributions, experiment eligibility, and future training snapshots.
...
- Published profile/item representation artifacts share exact snapshot, feature, model, evidence-watermark, integrity, personalization-settings, and privacy generations; stale or deleted generations cannot serve or reappear after restore.
...
Roadmap tickets live in `docs/roadmap/content-discovery/feat-368-*.md` through `feat-396-*.md`; `feat-447` is the live hybrid pilot, `feat-448` is the learned sequential profile/item representation, and `feat-449` is personalized row/page orchestration. Each ticket carries its U-ID scope, exact dependencies, reciprocal `blocks` links, Admin Evidence Gate, entry-point paths, grep terms, constraints, and focused verification. A candidate-generator ticket is complete after its shadow decision. If that decision is `promote-to-experiment`, create a new controlled-exposure ticket at that time rather than pre-authorizing live influence in the shadow ticket.
