...
- **Goal:** Expose the approved multi-interest profile source to a bounded consenting cohort through one semantic-plus-profile pipeline while semantic remains Essential-only behavior, experiment control, and last-known-good fallback.
...
- **Dependencies:** U17-U19. U18 proves bounded rollback before non-zero exposure, and U19 records `promote-to-experiment` for the exact profile generator manifest.
...
- **Approach:** Retrieve semantic and exact-manifest profile nominations in parallel, union source provenance, apply eligibility/ranking/composition once, and compose six unique playable titles when inventory permits. Read only bounded published profile generations; selection affects consented short-lived intent, while durable influence requires a consented qualified finalized outcome. Preserve sticky assignment, actual exposure, kill switch, privacy-generation fencing, semantic refill/fallback, and the unchanged 1.5-second complete-service contract.
- **Test scenarios:** Essential-only semantic delivery without profile access; consented same-session and qualified durable adaptation; selection without qualified playback; exact-six refill; current/recent-repeat suppression; sparse/stale/withdrawn/deleted profile fallback; idempotent assignment/exposure/evidence; concurrent withdrawal and projection publication; machine/test exclusion; and accessible low-bandwidth Watch controls.
- **Verification:** Use a vector-bearing production snapshot for cold/warm latency. Reconcile Watch request, assignment, final contributors, impression, selection, successful start, finalized outcome, superseding profile generation, later request, and semantic fallback in Admin; independently prove holdout, kill switch, withdrawal, erasure, and last-known-good rollback.
