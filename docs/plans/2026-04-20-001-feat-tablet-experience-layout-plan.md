...
- `apps/mobile/src/components/sections/ContainerRenderer.tsx` — **the only existing width-driven breakpoint in the codebase** (`STACK_BREAKPOINT = 500` at line 30). Below → stack, above → `flex: slot.gridSpan`. This is the direct analogue for the new `useBreakpoint()` hook: consult `useWindowDimensions()` inside the component, switch layout without prop-drilling.
- `apps/mobile/src/components/sections/CuratedHomeLayout.tsx` — renders a three-layer hero + single-column FlashList feed. Hero height is `screenWidth * 1.2` (line 36). FlashList uses the default single column. `@shopify/flash-list@^2.0.2` is used here and only here; no `numColumns`, `masonry`, `estimatedItemSize`, or `drawDistance` anywhere in the repo.
...
- `docs/solutions/mobile/android-lazy-section-viewport-gating-oom-fix.md` — Android decoder budget is 3–5 hardware slots on mid-range devices. Multi-column tablet layouts will mount more sections per viewport, making the budget tighter. Preserve `LazySection` buffers; revisit column counts on Android tablets specifically.
...
- **Android decoder budget ceiling in multi-column mode** — need to verify `LazySection` buffers still protect against OOM when the feed is 2-column. If Android tablets show decoder-starved flicker, introduce a tablet-aware buffer bump.
...
- Respect `LazySection` viewport gating (`android-lazy-section-viewport-gating-oom-fix`) — pass through any existing prop the current FlashList render path uses. Don't disable gating for tablet.
...
- **Integration — Android decoder budget**: Scrolling a 2-col feed on a mid-range Android tablet emulator (e.g. Pixel Tablet) does not trigger decoder-starved flickering or OOM. If it does, bump the `LazySection` buffer for tablets.
...
| Android tablet decoder-budget starvation under 2-col video feed. | Preserve `LazySection` viewport gating, add tablet-specific buffer bump if Unit 6 surfaces OOM / flicker. Captured in Deferred open question. |
