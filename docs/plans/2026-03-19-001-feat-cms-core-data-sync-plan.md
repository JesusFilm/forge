---
title: "feat: CMS Core Data Sync"
type: feat
status: active
date: 2026-03-19
origin: docs/brainstorms/2026-03-19-cms-core-sync-requirements.md
---
...
3. **Cron task** in `config/server.ts` with configurable schedule via `CORE_SYNC_CRON` env var
4. **Custom API endpoint** (`POST /api/core-sync/trigger`) for manual triggers
5. **Upsert logic** matching records by a `coreId` field (string, unique index)
6. **Soft-delete pass** after each phase completes, with circuit-breaker protection
7. **Source tagging** (`source: "core" | "manager"`) to protect manager-enriched records
...
config/cron-tasks.ts (schedule → calls sync service)
       ↓
src/api/core-sync/
  ├── services/core-sync.ts    -- orchestrator (runFullSync)
  ├── services/core-client.ts  -- fetch wrapper for gateway GraphQL
  ...
  ├── controllers/core-sync.ts -- manual trigger endpoint
  └── routes/core-sync.ts      -- POST /api/core-sync/trigger
```
...
**Common fields on all core-sourced collection types:**

- `coreId` (string, required, unique) — match key for upserts
- `source` (enumeration: `core`, `manager`, default: `core`) — ownership tag (see origin: R12)
...
**Non-localized fields** (shared across all locales): coreId, slug, label, source, published, locked, noIndex, publishedAt, and all relation fields.
...
- [x] Create `Language` content type schema (`src/api/language/content-types/language/schema.json`) with: coreId (string, unique, required), bcp47 (string), iso3 (string), slug (string), source (enumeration). i18n enabled, `name` field localized.
...
- [x] Create `Continent` content type schema with: coreId, source. i18n enabled, `name` field localized.
- [x] Create `Country` content type schema with: coreId, population (integer), latitude (float), longitude (float), flagPngSrc (string), flagWebpSrc (string), languageCount (integer), languageHavingMediaCount (integer), source. i18n enabled, `name` field localized. Relation: continent (manyToOne).
- [x] Create `CountryLanguage` content type with: coreId, speakers (integer), displaySpeakers (integer), primary (boolean), suggested (boolean), order (integer), source. Relations: language (manyToOne), country (manyToOne).
- [x] Create `BibleBook` content type with: coreId, osisId (string), alternateName (string), paratextAbbreviation (string), isNewTestament (boolean), order (integer), source. i18n enabled, `name` field localized.
- [x] Create `VideoOrigin` content type with: coreId, name (string), description (text), source.
- [x] Create `VideoEdition` content type with: coreId, name (string), source.
- [x] Create `MuxVideo` content type with: coreId, assetId (string), playbackId (string), duration (integer), readyToStream (boolean), downloadable (boolean), primaryLanguageId (string), source.
- [x] Create `CloudflareR2` content type with: coreId, contentLength (biginteger), contentType (string), fileName (string), originalFilename (string), publicUrl (string), source.
- [x] Create `Keyword` content type with: coreId, value (string), source. Relation: language (manyToOne).
- [x] Create `VideoVariant` content type with: coreId, slug (string), duration (integer), lengthInMilliseconds (biginteger), hls (string), dash (string), share (string), downloadable (boolean), published (boolean), version (integer), brightcoveId (string), source. Relations: language, videoEdition, muxVideo, asset (CloudflareR2).
...
- [x] Create `VideoSubtitle` content type with: coreId, primary (boolean), vttSrc (string), srtSrc (string), vttVersion (integer), srtVersion (integer), value (string), edition (string), source. Relations: language, videoEdition, vttAsset (CloudflareR2), srtAsset (CloudflareR2).
...
- [x] Create `config/cron-tasks.ts` with configurable schedule via `CORE_SYNC_CRON` env var (default: `0 3 * * *` — daily at 3am)

**Success criteria:** All content types registered in Strapi admin, cron fires on schedule.

#### Phase 2: Core Client and Language Sync

Build the GraphQL client and the first sync phase (languages), including dynamic i18n locale registration.

**Tasks:**

- [x] Create `src/api/core-sync/services/core-client.ts` — Apollo Client wrapper for gateway GraphQL (replaced native fetch with Apollo Client + codegen typed documents)
- [x] Create `src/api/core-sync/services/sync-languages.ts`:
  ...
  3. Upsert Language records by coreId
...
- [x] Create `src/api/core-sync/services/core-sync.ts` — orchestrator with phased execution, selective scope, in-memory lock, and summary logging
...
- [x] Create `src/api/core-sync/services/sync-countries.ts`:
...
- [x] Create `src/api/core-sync/services/sync-videos.ts`:
  1. First pass — upsert reference types: BibleBook, VideoOrigin, VideoEdition (extracted from video responses)
  2. Paginated video loop (page size configurable via `CORE_SYNC_VIDEO_PAGE_SIZE`, default 100):
     ...
     - Store children as JSON core ID arrays
     - Progress logging with count/total percentages
  3. Soft-delete pass with circuit breaker
- [x] Create `src/api/core-sync/services/sync-video-variants.ts` (split from videos for clarity):
...
**Children/Parents strategy:** Store as JSON string arrays (`childCoreIds`, `parentCoreIds`) on the Video content type. This avoids forward-reference issues during pagination — a video on page 1 can reference a child on page 50 without that child existing yet. Consuming apps resolve references via a second query by coreId. (see origin: R3 "stored as references (core IDs)")
...
- [x] Create `src/api/core-sync/controllers/core-sync.ts` with `trigger` (202 Accepted, fire-and-forget) and `status` actions
- [x] Create `src/api/core-sync/routes/core-sync.ts` with `POST /trigger` and `GET /status` (both admin-authenticated)
- [x] Wire cron task in `config/cron-tasks.ts` to call `runFullSync()` on configurable `CORE_SYNC_CRON` schedule
...
- [x] Redesign `src/api/video/content-types/video/schema.json` — replaced with full core sync schema. `slug` changed from `uid` to `string`.
- [ ] Update `src/bootstrap/seed-easter.ts` — currently disabled with TODO comment. Needs rewrite to reference core-synced videos by `coreId` or tag as `source: "manager"`.
...
Sync trigger (cron or API) → `core-sync.runFullSync()` → sequential calls to `sync-languages`, `sync-countries`, `sync-videos` → each calls `core-client` for fetches → each calls `strapi.documents()` for upserts → Strapi fires content-type lifecycle hooks (beforeCreate, afterCreate, etc.) → Strapi auto-rebuilds GraphQL schema if content types change at boot → `packages/graphql` codegen must be re-run after schema changes → `apps/web` and `apps/mobile` queries may need updating.

### Error Propagation

- Gateway fetch errors (network, timeout, 5xx) → caught by core-client retry logic → after 3 retries, bubbles up to orchestrator → orchestrator logs error, releases lock, reports partial failure
...
- [x] Manual sync can be triggered via `POST /api/core-sync/trigger` (authenticated)
- [x] Cron schedule is configurable via `CORE_SYNC_CRON` environment variable
...
- [x] Structured logging with `[core-sync]` prefix for all sync operations
...
| Easter seeder creates slug conflicts with core-synced data   | Medium     | Unique constraint errors               | Rewrite seeder to lookup by coreId or tag as `source: "manager"` |

## Sources & References

### Origin

- **Origin document:** [docs/brainstorms/2026-03-19-cms-core-sync-requirements.md](docs/brainstorms/2026-03-19-cms-core-sync-requirements.md) — Key decisions: upsert by core ID, Strapi i18n for translations, replace existing Video type, soft-delete on removal, source tagging for manager enrichment
