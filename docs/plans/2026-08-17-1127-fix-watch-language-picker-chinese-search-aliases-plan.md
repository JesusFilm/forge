...
- R4. Direct display-name and native-name matches must rank ahead of alias-only matches; results within the same tier must preserve the picker's original order.
...
- F1. Find a language by a familiar Chinese name
  ...
  - **Steps:** The picker checks only its supplied options, ranks direct name matches ahead of alias-only matches, and displays the matching subset.
...
- AE4. **Covers R4, R11.** Given that the current picker contains several Chinese-group members, when A1 enters `中文`, direct display-name or native-name matches rank ahead of alias-only matches, and remaining ties preserve original order.
...
- KTD3. **Treat exact configured aliases as authoritative and rank partial alias matches after every direct tier.** If a normalized query exactly equals a configured alias, only selectable supplied options whose exact slug owns that alias are eligible; BCP-47-derived native labels and disabled context rows cannot bypass this gate. For other queries, display-name and native-name matching keeps the existing prefix, word-prefix, and substring tiers, followed by alias prefix, word-prefix, and substring tiers; original option order breaks same-tier ties. Governs R3-R5, R11-R13.
...
- **Files:**
  ...
  - `apps/web/src/components/watch/__tests__/GlobalLanguagePickerModal.aliases.test.tsx`
...
- **Patterns to follow:**
  ...
  - Preserve the direct-match priority defined by `docs/roadmap/platform/feat-169-watch-language-picker-search-ranking.md`.
- **Test scenarios:**
  ...
  6. Alias-only ties preserve caller order.
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Roadmap               | `pnpm --filter roadmap generate:readme`                                                                                                                                                                                                                                                                                                           | The new ticket is listed, the README has no drift, and the ticket is `complete` after all verification passes.                                                                                                                                                         |
| Focused tests         | `pnpm --filter @forge/web test -- src/lib/watch-language-search-aliases.test.ts src/components/watch/__tests__/LanguageCombobox.test.tsx src/components/watch/__tests__/GlobalLanguagePickerModal.test.tsx src/components/watch/__tests__/GlobalLanguagePickerModal.aliases.test.tsx src/components/watch/__tests__/LanguagePickerModal.test.tsx` | Alias, ranking, availability-boundary, keyboard, and three-picker integration scenarios for U1-U2 pass.                                                                                                                                                                |
...
- The roadmap ticket is `in-progress` before implementation, `complete` after all verification passes, and `docs/roadmap/README.md` is regenerated.
