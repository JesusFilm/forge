...
- KTD6. **Ship the container field undeclared in the collection schema, and bump BOTH revisions.** `watchCatalogCollectionSchema` declares 7 fields while `TypesenseWatchCatalogDocument` has 15 — `coreId`, `label`, `imageUrl`, `imageBlurDataUrl`, `localesJson`, the two options blobs, and `id` are stored and returned undeclared — and U0 confirmed the JSON-string carrier round-trips undeclared, so the collection's field manifest is unchanged. That does **not** license leaving the application revision alone. The manifest answers whether Typesense accepts the field; the revision answers whether an older generation can still serve today's code. A generation built before this change carries documents with no such key, so every container in it resolves `unavailable` — the exact defect this work removes — and an unbumped revision would leave that generation compatible, free to requalify and be promoted with the benchmark green while serving the bug. The application revision therefore moves to `watch-search-candidate/v3`, forcing a fresh generation, and the ranking revision moves because the rank ladder changed. Governs R7.
...
**Measured 2026-08-28 against Typesense 30.2** (the pinned local build, via `scripts/typesense-watch-search-local.sh start`): an `include_fields` list naming a field that **no document in the collection carries returns HTTP 200 and omits it** — not a 400. The same probe confirmed a JSON-string field round-trips undeclared, while an undeclared object array is projected inconsistently, which is why the carrier is a JSON string (KTD2).

That selects the first row below. Re-verify on a major Typesense upgrade; the second row is retained because it is what the rollout becomes if that behaviour ever changes.

| U0 question 2 result                                               | Deploy sequence                                                                                                                        | Error-classifier action                                                                                                                                                          |
| ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Measured:** `include_fields` ignores a field no document carries | Single deploy, then operator rebuild. Field-less documents resolve `unavailable` per R13 — today's behavior — until the rebuild lands. | None needed.                                                                                                                                                                     |
| If a future version errors instead                                 | Deploy U3 **without** the include-list additions, run the operator rebuild, then a second deploy adds the include-list entries.        | Widen the hydration error classifier to treat a catalog-side field-projection failure as a recoverable fallback rather than a rethrow, so the window cannot 500 the search path. |
...
- **Approach:** Bump both revisions and record why at each constant. The ranking revision moves because the rank ladder gained a tier; the application revision moves to `watch-search-candidate/v3` because the catalog document projection and the catalog retrieval-field contract both changed, and a generation predating that change would otherwise stay compatible while serving every container as unavailable (KTD6). Then run the candidate qualification and confirm it reports no added Typesense round-trips and no latency-percentile regression — the benchmark fails closed on both, and the revision rules are only meaningful if it has actually run.
- **Test scenarios:**
  - The ranking revision differs from its previous value.
  - The application revision reads `watch-search-candidate/v3`.
  - The two revisions remain independently variable, so changing either alone moves the evaluation identity.
...
- Both revisions are bumped and justified in place, and the candidate qualification has run with no added round-trips and no latency regression.
