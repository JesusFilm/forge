...
- _Covers AE3._ Structured output parses successfully → quality-draft
...
"provider_unavailable"}` → service yields the same error event;
  no fallback to another channel. _Covers AE5._
- Edge case: `provider="garbage"` → normalizes to `openrouter` → no
  throw, behavior matches default.
- _Covers AE2:_ Same thread picks `ollama` for a quality-draft turn
...
- _Covers AE6:_ option labels indicate cost posture (test asserts
