...
WITH r AS MATERIALIZED (
 ...
 WHERE r.created_at >= '2026-09-08T01:45:00Z' AND r.created_at < '2026-09-15T01:45:00Z'
 AND r.expires_at > '2026-09-15T02:03:52.496Z'
), o AS MATERIALIZED (
 ...
 WHERE o.created_at < '2026-09-15T01:45:00Z'
 ...
 (e.claimed_at IS NOT NULL AND e.claimed_at<'2026-09-15T01:45:00Z') claimed,e.id episode_id,
 EXISTS(SELECT 1 FROM recommendation_playback_fact pf WHERE pf.episode_id=e.id AND pf.kind='playback_start' AND pf.received_at<'2026-09-15T01:45:00Z') started,
 ...
 LEFT JOIN recommendation_rendered_fact rf ON rf.item_id=i.id AND rf.received_at<'2026-09-15T01:45:00Z'
 LEFT JOIN recommendation_impression im ON im.item_id=i.id AND im.received_at<'2026-09-15T01:45:00Z'
 LEFT JOIN recommendation_selection s ON s.item_id=i.id AND s.received_at<'2026-09-15T01:45:00Z'
 LEFT JOIN recommendation_playback_episode e ON e.selection_id=s.id AND e.created_at<'2026-09-15T01:45:00Z'
...
SELECT execution_mode,candidate_generator,count(item_id) cards,count(*) FILTER(WHERE impressed) impressions,count(*) FILTER(WHERE selected) selections,count(*) FILTER(WHERE selected_after_impression) matched_selections,count(*) FILTER(WHERE started) starts,count(*) FILTER(WHERE active_qualified) qualified,coalesce(sum(active_ms),0) active_ms FROM f WHERE item_id IS NOT NULL GROUP BY 1,2 ORDER BY 1,2;
