...
SELECT position,count(item_id) items,count(*) FILTER(WHERE impressed) impressions,count(*) FILTER(WHERE selected) selections,count(*) FILTER(WHERE selected_after_impression) selected_after_impression,count(*) FILTER(WHERE active_qualified) active_qualified FROM f WHERE item_id IS NOT NULL GROUP BY 1 ORDER BY 1;
