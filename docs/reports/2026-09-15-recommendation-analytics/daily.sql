...
SELECT to_char(created_at AT TIME ZONE 'UTC','YYYY-MM-DD') AS utc_date,count(distinct request_id) requests,count(distinct session_digest) sessions,
 ...
 coalesce(sum(active_ms),0) active_ms FROM f GROUP BY 1 ORDER BY 1;
