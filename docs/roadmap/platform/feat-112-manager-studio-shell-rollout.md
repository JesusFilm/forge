...
The Studio shell direction existed outside the real manager app, which still used the older dashboard frame, DOM-based header injection, and a separate login look. That left Coverage, Jobs, Job Detail, and Agents visually inconsistent and made shell-level controls harder to share across routes.
...
4. Apply the Studio shell to coverage, jobs, job detail, and agents without broadly restyling each page interior.
...
2. Replaced the legacy dashboard layout with the Studio shell, including real-route nav for Coverage, Jobs, and Agents.
