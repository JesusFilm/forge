...
2. Expose a bounded, cursor-paginated public Admin GraphQL feed of visible
   collection parents that have visible playable children.
...
4. Render the special block as a client-side infinite feed. Fetch when a
   sentinel nears the viewport, append collection carousels, stop at the end,
   offer a retry on failure, and prevent duplicate in-flight requests.
...
- The canonical footer remains immediately after the authored homepage and
  before the dynamic feed, so primary navigation stays reachable while the
  discovery surface keeps growing.
