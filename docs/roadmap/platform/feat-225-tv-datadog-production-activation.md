...
1. **Provision EAS env vars per profile** (`eas env:create`): `EXPO_PUBLIC_DATADOG_CLIENT_TOKEN` (the `pub...` client token from the "Forge TV" RUM application in Datadog — Digital Experience -> RUM -> Applications), `EXPO_PUBLIC_DATADOG_APPLICATION_ID` (same page), `EXPO_PUBLIC_DATADOG_SITE=US1`. Leave `EXPO_PUBLIC_DATADOG_ENV` unset — it defaults by build type (`__DEV__` -> development, release -> prod). Set `EXPO_PUBLIC_DATADOG_VERSION` from the build's git SHA if available in the profile.
