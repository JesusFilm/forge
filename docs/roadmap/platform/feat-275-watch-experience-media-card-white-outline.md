...
- Focused Vitest coverage passes: 12 files and 115 tests, including direct
  contracts for Watch home hero, generated sections, language inventory,
  history, active and pending states, keyboard activation, and fallback
  behavior. Unroutable episode and sibling cards also cannot activate or mount
  a Mux preview.
...
- A local Chromium proof rendered the production `WatchHomeHero` component and
  confirmed both the active and keyboard-focus-visible frames at 4px solid
  white with opacity 1. The full data-backed local Watch route remained blocked
  by the existing local Admin/Postgres extension mismatch; the Railway Web
  deployment check passed.
- The shared frame adds no effects, listeners, requests, media work, or client
  state; it is one inert `pointer-events-none` span rendered only for
  interactive or explicitly active cards.
- Structured correctness, testing, maintainability, standards, adversarial,
  agent-native, and learnings reviews have no remaining actionable findings.
