...
- TypeScript syntax transpilation, scoped Prettier validation, and
  `git diff --check` pass. Focused Vitest, package typecheck/lint, and local
  browser smoke could not run because the worktree's existing dependency tree
  was incomplete; cached restoration did not complete and no package or
