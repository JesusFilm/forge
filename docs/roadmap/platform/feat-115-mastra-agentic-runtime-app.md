...
Forge has Manager-side agents and automations, but there is no first-class Mastra runtime app in the monorepo. If Manager embeds Mastra directly, future apps that need to create or consume agents and workflows will either duplicate runtime code or cross-import Manager internals, which violates app boundaries.

Add `apps/mastra` as the shared agentic runtime and Mastra Studio app. Manager should be the first consumer, but the runtime boundary should be reusable by future Forge apps.

## Entry Points -- Read These First

1. `docs/brainstorms/2026-05-01-mastra-agentic-runtime-app-requirements.md` -- chosen product shape and boundaries
...
1. Create `apps/mastra` as a new monorepo app with package name `@forge/mastra`.
...
5. Define a narrow Manager integration contract for the first Mastra-powered flow.
...
7. Document how future apps should consume Mastra agents/workflows without cross-importing app internals.
...
- Do not duplicate canonical content or enrichment job truth in Mastra; Strapi/CMS and Manager keep their current ownership.
...
- `apps/mastra/package.json` is named `@forge/mastra` and exposes standard `lint`, `typecheck`, `test`, `build`, `dev`, and `start` scripts as applicable.
- `pnpm --filter @forge/mastra lint`, `pnpm --filter @forge/mastra typecheck`, `pnpm --filter @forge/mastra test --if-present`, and `pnpm --filter @forge/mastra build` pass or have documented first-slice exceptions.
- Mastra Studio is reachable only through the intended internal/operator path and is not anonymously public.
- Manager can invoke the first Mastra-backed flow through an explicit contract without importing app internals.
