...
artifactType: issue
issueNumber: 73
issueTitle: "feat(ci): automate cms terraform plan/apply with turbo affected gating"
issueUrl: "https://github.com/JesusFilm/forge/issues/73"
state: "CLOSED"
closedAt: "2026-03-08T22:01:08Z"
labels: []
linkedPrs:
  [
...
Epic #67 needs a safe and deterministic CI pathway for Terraform plan/apply and CMS deploy execution. Deploys should run only when relevant changes are detected.

## Expected outcome

GitHub Actions workflows run Terraform checks/plans on PRs and controlled applies on `stage`/`main`, with approvals and Turbo affected gating to prevent unnecessary deployments.

## Acceptance criteria

- [ ] PR workflow runs `fmt`, `validate`, `plan` for relevant infra changes.
- [ ] `stage` branch can apply stage infra/deploy when relevant changes exist.
- [ ] `main` branch apply to prod requires protected environment approval.
- [ ] Turbo affected gating is implemented for CMS/infra relevance checks.
- [ ] AWS auth uses OIDC role assumption (no long-lived cloud keys).
- [ ] Workflow logs clearly show why deploy did/did not run.

## Possible solution(s)

1. Extend existing `terraform-plan.yml` and add dedicated apply workflow.
2. Add pre-step that computes affected targets and changed infra paths.
3. Use GitHub Environments + required reviewers for prod gate.
