...
artifactType: issue
issueNumber: 144
issueTitle: "feat(cms): add schema for Text component"
issueUrl: "https://github.com/JesusFilm/forge/issues/144"
state: "CLOSED"
closedAt: "2026-03-04T20:55:24Z"
labels: ["cms", "feat"]
linkedPrs:
  [
...
Need a dedicated Text component schema so editors can add rich or plain text blocks (headings, paragraphs, lists) within sections or containers in the CMS.

## Expected outcome

- A Text component exists with attributes for content (e.g. richtext or string), optional heading level, and any display options.

## Acceptance criteria

- [ ] Text component JSON schema added in CMS.
- [ ] Component registered and available in sections/containers; GraphQL regenerated if contracts change.

## Possible solution(s)

1. Add `components/shared/text.json` or `sections/text.json` with `content` (type: richtext or text) and optional `heading`, `variant`.
2. If Strapi richtext is used, ensure blocks/components are aligned with web rendering.
...
- Related (web implementation): #154
