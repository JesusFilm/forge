...
artifactType: issue
issueNumber: 95
issueTitle: "chore(mobile-expo): Dev/stage/prod config, README, gitignore"
issueUrl: "https://github.com/JesusFilm/forge/issues/95"
state: "CLOSED"
closedAt: "2026-03-04T00:49:10Z"
labels: []
linkedPrs:
  [
...
The Expo app needs environment config (dev/stage/prod, e.g. GraphQL URL), a README with build/run instructions, and a .gitignore appropriate for Expo/Node so the repo stays clean and contributors can run the app.

## Expected outcome

- Environment configuration for dev, stage, and prod (e.g. env files or Expo config); GraphQL endpoint and any API tokens (no secrets in repo).
- README under `mobile/expo` (or linked from `mobile/README.md`) with: how to install deps, run on iOS/Android, and point at dev/stage/prod.
- .gitignore entries for Expo/Node (e.g. node_modules, .expo, build artifacts).

## Acceptance criteria

- [ ] Dev/stage/prod config documented and used by app (e.g. GraphQL URL).
- [ ] README describes build and run steps.
- [ ] .gitignore covers Expo and Node artifacts; no secrets committed.

## Possible solution(s)

1. Use Expo env (e.g. app.config.js with env vars) or .env files (gitignored) with .env.example.
2. Extend root or mobile .gitignore for `mobile/expo` if needed.

## References

- Parent: #89
- Depends on: #90 (scaffold)
