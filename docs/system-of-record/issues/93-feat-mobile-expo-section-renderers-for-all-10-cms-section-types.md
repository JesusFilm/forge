...
artifactType: issue
issueNumber: 93
issueTitle: "feat(mobile-expo): section renderers for all 10 CMS section types"
issueUrl: "https://github.com/JesusFilm/forge/issues/93"
state: "CLOSED"
closedAt: "2026-03-11T00:46:35Z"
labels: []
linkedPrs:
  [
...
The watch app must render each section type returned by the CMS. The schema defines 10 active section types in the `ExperienceSectionsDynamicZone` union. This issue is the **tracking parent** for all section renderer sub-issues.

> **Note:** PromoBanner is superseded by VideoHero; InfoBlocks is superseded by Text. Neither should be implemented.
...
This issue is complete when all 11 sub-issues above are closed:

- [ ] All 10 section types have functional renderers.
- [ ] SectionDispatcher correctly routes all `__typename` values.
- [ ] Nested content rendering works for Section and Container wrappers.
- [ ] Components accept typed props from the section data layer.
- [ ] Layout and content align with jesusfilm.org/watch experience pages.
- [ ] Accessible and responsive where applicable.
...
- Parent: #89
- Depends on: #304 (expanded data layer)
- [apps/cms/schema.graphql](apps/cms/schema.graphql) — `ExperienceSectionsDynamicZone` union (line 693)
- [mobile/ios/GraphQL/Operations/GetWatchExperience.graphql](mobile/ios/GraphQL/Operations/GetWatchExperience.graphql) — reference query
- [apps/web/src/components/sections](apps/web/src/components/sections) — web section components for reference
