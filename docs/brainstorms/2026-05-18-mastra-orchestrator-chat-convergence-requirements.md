...
- R9. These env vars are removed from `apps/admin/src/config/env.ts` and `apps/admin/.env.example`: `EXPERIENCE_AI_ALLOW_CODEX`, `EXPERIENCE_AI_ALLOW_CODEX_FALLBACK`, `EXPERIENCE_AI_ALLOW_CLAUDE_CODE`, `EXPERIENCE_AI_CODEX_MODEL`, `EXPERIENCE_AI_CLAUDE_CODE_MODEL`, `OLLAMA_BASE_URL`, `OLLAMA_CHAT_MODEL`, `OLLAMA_EMBEDDING_MODEL`, `OLLAMA_EMBEDDING_DIMENSIONS`.
