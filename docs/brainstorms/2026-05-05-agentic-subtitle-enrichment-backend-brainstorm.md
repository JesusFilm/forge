# Mastra Subtitle Enrichment Backend Brainstorm

Date: 2026-05-05

## What We're Building

Move the subtitle enrichment workflow execution out of Manager and into Mastra, backed by Mastra workflows. Manager remains the product-facing consumer: it keeps the coverage UI, automation UI, job list/detail views, approvals, and CMS-backed job truth, but it no longer runs the long-lived subtitle workflow in-process.
...
This does not move the full video enrichment pipeline yet. Chapters, metadata, embeddings, audio cleanup, scene analysis, voiceover, and broader content generation stay on their current Manager paths until the subtitle backend proves the Mastra workflow boundary.

## Why This Approach

The existing Manager pipeline already has the right operator experience and job truth, but the workflow execution is app-local. That makes Mastra less reusable and keeps future apps from consuming the same agent/workflow platform without either duplicating orchestration or importing Manager internals.

Mastra is the better home for runtime concerns:
...
4. A single service boundary for future mastra workflows.
...
The clean split is: Manager decides what work should happen and shows what happened; Mastra executes the subtitle workflow and reports back through typed contracts.
...
Mastra currently has only the Manager automation dry-run contract. That is useful precedent for service auth and typed Manager callbacks, but it is not yet a live subtitle workflow backend.

## Related Compound Docs Checked

1. `docs/brainstorms/2026-05-01-mastra-runtime-app-requirements.md` - Mastra app boundary and Manager-first consumer model.
2. `docs/roadmap/platform/feat-115-mastra-runtime-app.md` - Mastra runtime ownership and cross-app constraints.
...
Manager should expose a small "start subtitle enrichment" consumer contract to Mastra instead of embedding Mastra. From the user's point of view, the existing Manager controls keep working:

1. Operators select videos and target language from Coverage.
2. Manager creates or reuses a CMS `EnrichmentJob`.
3. Manager sends a service request to Mastra with a job id, video identity, source language, target language, artifact context, and idempotency key.
4. Mastra starts a Mastra workflow run and stores the Mastra run id.
5. Mastra reports step status and artifact metadata back to Manager.
6. Manager renders the existing job list and job detail views from CMS state.

For automations, Manager still selects eligible videos and enforces caps. Mastra should execute only the work Manager asks it to execute. That preserves the current dry-run and approval boundaries while making the runtime reusable.
...
Manager should not run the workflow implementation, but it should still own the CMS-backed job record, operator UX, automations, approvals, and coverage decisions. "Pure consumer" means Manager consumes Mastra workflow APIs instead of importing or hosting Mastra workflow code.

### 3. Mastra owns execution state and orchestration

Mastra owns the Mastra workflow definition, run ids, runtime storage, retries, traces, Studio visibility, tool registry, and step execution. Future apps should be able to call the same Mastra workflow boundary without importing Manager internals.

### 4. Cross-app communication stays HTTP-only

Do not cross-import Manager workflow code into Mastra or Mastra workflow code into Manager. If shared types become necessary, introduce a deliberately scoped package or generated contract after planning proves it is worth the maintenance cost.
...
Dry-run should continue to produce reports without creating live jobs or mutating artifacts. Live mode should require an explicit Manager-created job plus an idempotency key. Mastra should reject live requests that do not reference a Manager-approved job.

### 8. Side effects stay constrained

Mastra may execute transcription, translation, and Mux sync steps, but live mutations should be behind narrow Manager/CMS/Mux service contracts with typed failures. Mastra should not gain broad CMS mutation power just because it can run a workflow.
...
Add the Mastra subtitle workflow API, schemas, auth, idempotency, and a no-op Mastra workflow that reports start and completion back to Manager. Manager still has a feature flag that keeps the current local workflow path as fallback.

### Phase 2: Move subtitle execution

Move or re-home the source transcription and target subtitle translation execution behind Mastra tools. Keep artifact outputs identical. Manager starts Mastra runs instead of `launchVideoEnrichment(...)` for subtitle-only jobs.

### Phase 3: Move subtitle publication

Let the Mastra workflow perform the Mux subtitle sync through a narrow Manager-approved service contract. Keep override and recovery UX in Manager.

### Phase 4: Retire Manager-local subtitle orchestration

Once Mastra status, artifacts, retries, and failure semantics match Manager behavior, remove the Manager-local subtitle workflow path for the migrated cases. Leave non-subtitle enrichment steps untouched.

## Non-Goals

1. Do not migrate chapters, metadata, embeddings, scene analysis, audio cleanup, or voiceover in this slice.
2. Do not make Mastra the canonical content store.
3. Do not let Mastra select automation candidates.
...
1. Duplicate job truth if Mastra stores business state instead of runtime state.
2. Lost operator context if Manager stops receiving granular step and artifact events.
3. Unsafe retries if idempotency is weak across Manager and Mastra boundaries.
4. Artifact drift if Mastra changes canonical file names or metadata formats.
5. Over-broad service auth if Mastra receives general CMS mutation credentials.
6. Confusing rollback if Manager cannot fall back to the local workflow during rollout.

## Validation Ideas

1. Contract tests for Manager-to-Mastra run creation and Mastra-to-Manager events.
2. Red/green test showing Manager starts Mastra for subtitle-only jobs behind the feature flag.
3. Red/green test showing dry-run does not create jobs or artifacts.
4. Idempotency test showing duplicate Manager requests return the same Mastra run.
...
6. User smoke test from Coverage: select one video and one target language, start enrichment, then watch the Manager job detail page update from Mastra events.
7. Operator smoke test for Mastra Studio: confirm the Mastra run appears with step trace and cannot be accessed without operator auth.
