...
- AE1. _(covers R1, R2, R8)_ With the dropdown at `openrouter`,
...
- AE2. _(covers R2, R7)_ Picking `ollama`, the persisted assistant
  row stamps `providerKind: "ollama-gemma4"` for both quality-draft
  and chat-turn flows in the same thread.
- AE3. _(covers R3, R4)_ Picking `codex` on a quality-draft turn,
  ...
  error path.
- AE4. _(covers R5, R10)_ With `EXPERIENCE_AI_ALLOW_CLAUDE_CODE`
...
- AE5. _(covers R6)_ Each provider's failure modes (timeout,
  upstream error, validation failure) map onto the existing
  `ChatErrorCode` literals. The chat panel's error rendering is
  unchanged.
- AE6. _(covers R9)_ The dropdown's option labels indicate cost
