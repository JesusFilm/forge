...
| `?`                        | Parity status genuinely unknown. Resolved at two layers: (a) typename-level by this inventory's U4 mapping table (which classifies 4 typenames as `adapter-required` and leaves 13 at `?`); (b) field-level by the **parent migration plan's Unit 4 (parity harness)**, which is downstream of this whole document. Field tables in U1/U2/U3 retain `?` until the parity harness runs. |
...
| ---------------------- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `videoRecommendations` | top-level                  | Forward-looking R5 block; schema-only, no editor UX, no renderer yet (`blocks.ts:361-377`). Powered at render time by `sceneRecommendations(videoId \| slug, locale, limit)`. Authored only via admin; will never appear as a Strapi `__typename`. |
