...
| Admin search eval caller bearer         | `apps/admin` scripts                   | `SEARCH_API_KEY` caller-side value                                    | Search eval CLI -> Admin search                                 | CLI caller credential, expected to match partner or known internal key | Caller-side only; receiver branch for `SEARCH_API_KEYS` is retired               | Keep        |
...
| Surface                             | Owner                | Credential / Config                         | Receiver / Consumer                   | Principal / Meaning                   | Current Posture                                              | Disposition |
| ----------------------------------- | -------------------- | ------------------------------------------- | ------------------------------------- | ------------------------------------- | ------------------------------------------------------------ | ----------- |
| Admin search eval bearer            | `apps/admin`         | `SEARCH_API_KEY`                            | Admin search                          | CLI known-caller passport             | Optional during dual-accept; required after search auth flip | Keep        |
