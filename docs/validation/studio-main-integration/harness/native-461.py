...
e.update(TMPDIR=str(b/'tmp'),CI='1',DATABASE_URL='postgresql://tataihono@127.0.0.1:55461/forge_studio_462_merge_test',REDIS_URL='redis://127.0.0.1:54630',NEXT_TELEMETRY_DISABLED='1',DD_TRACE_ENABLED='false',DD_INSTRUMENTATION_TELEMETRY_ENABLED='false',OTEL_SDK_DISABLED='true',ADMIN_SESSION_SECRET='studio462-owned-fixture-at-least-32-characters',AUTH_ISSUER_URL='http://127.0.0.1:55469',AUTH_ADMIN_CLIENT_ID='studio462-fixture',STUDIO_PRODUCTION_ENABLED='true',STUDIO_PUBLICATION_ENABLED='true')
e.update(WATCH_SEARCH_DB_TEST='1')
pg=Path('/usr/lib/postgresql/18/bin')
assert not (b/'pgdata/postmaster.pid').exists()
with socket.socket() as sock:sock.bind(('127.0.0.1',55461))
subprocess.run([str(pg/'pg_ctl'),'-D',str(b/'pgdata'),'-l',str(b/'logs/postgres.log'),'-o',f'-h127.0.0.1 -p55461 -k{b}/tmp','-w','start'],check=True,env=e,stdout=subprocess.DEVNULL)
e['STUDIO_TEST_DATABASE_URL']='postgresql://tataihono@127.0.0.1:55461/forge_studio_461_test'
subprocess.run([str(pg/'createdb'),'-h','127.0.0.1','-p','55461','-U','tataihono','forge_studio_461_test'],check=True,env=e)
try:
