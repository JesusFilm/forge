...
e.update(RECOMMENDATION_DB_TEST='1', RECOMMENDATION_DELIVERY_DB_FIXTURE='deterministic', RECOMMENDATION_PROFILE_DB_FIXTURE='deterministic')
pg=Path('/usr/lib/postgresql/18/bin')
assert not (b/'pgdata/postmaster.pid').exists()
with socket.socket() as sock:sock.bind(('127.0.0.1',55463))
subprocess.run([str(pg/'pg_ctl'),'-D',str(b/'pgdata'),'-l',str(b/'logs/postgres.log'),'-o',f'-h127.0.0.1 -p55463 -k{b}/tmp','-w','start'],check=True,env=e,stdout=subprocess.DEVNULL)
try:
