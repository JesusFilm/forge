...
const base='.tmp/studio461',next='apps/manager/.next',dataset=()=>execFileSync('psql',['postgresql://tataihono@127.0.0.1:55461/forge_studio_461_test','-Atc',"select md5(string_agg(id||':'||current_revision||':'||lifecycle,',' order by id))||'|'||count(*) from studio_project"],{encoding:'utf8'}).trim();
const host=()=>({at:new Date().toISOString(),load:loadavg(),freeMemory:freemem(),cpu:readFileSync('/proc/stat','utf8').split('\n')[0]});
const result={datasetBefore:dataset(),samples:[],viewport:{width:1440,height:1000},design:'Five AB/BA alternating rounds; same origin and backend; restarted variant process warmed by login each route; browser cold/warm; CPU profiler active identically; 1s post-ready observation'};
...
const persist=()=>writeFileSync(base+'/alternating-loading.json',JSON.stringify(result,null,2));
async function stop(){if(child){process.kill(-child.pid,'SIGTERM');await new Promise(r=>child.once('exit',r));child=null;}if(active){renameSync(next,next+'-studio461-'+active);active=null;}}
try{
for(let round=0;round<5;round++)for(const variant of round%2?['feature','baseline']:['baseline','feature']){
 renameSync(next+'-studio461-'+variant,next);active=variant;
 const log=openSync(base+`/alternating-server-${round}-${variant}.log`,'w');child=spawn('node',[base+'/runtime.mjs','start-native-manager','manager'],{detached:true,stdio:['ignore',log,log]});closeSync(log);
 ...
 for(const route of ['','/new']){
  ...
  const page=await context.newPage();await page.goto('http://127.0.0.1:3463/login');await page.getByRole('link',{name:/Calendar baseline project/}).waitFor();await page.waitForTimeout(1000);const cdp=await context.newCDPSession(page);await cdp.send('Network.clearBrowserCache');await cdp.send('Performance.enable');await cdp.send('Profiler.enable');
  for(const cache of ['cold','warm']){
   const before=host(),cpuBefore=await cdp.send('Performance.getMetrics');await cdp.send('Profiler.start');
   await page.goto('http://127.0.0.1:3461/dashboard/shorts'+route,{waitUntil:'load'});if(route)await page.getByRole('textbox').first().fill('Matched baseline typing');else await page.getByRole('link',{name:/Calendar baseline project/}).waitFor();
...
}catch(e){result.error=String(e);throw e}finally{await stop();renameSync(next+'-studio461-feature',next);result.finishedAt=new Date().toISOString();persist();await browser.close();}
