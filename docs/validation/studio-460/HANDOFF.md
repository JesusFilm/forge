...
## Retained worker shutdown follow-up

See [bounded shutdown](worker-shutdown/README.md) for delivered-signal diagnosis, the explicit five-second lifecycle, failure-first cleanup ownership tests, and exact-image provenance. Unconfirmed normal-job cleanup closes admission and retires unsuccessfully; cooperative cancellation remains reusable. The observed baked `/public` asset-path defect is corrected only in each job's copied bundle. This work does not resolve the Studio generated-code proc failure. The [plain-language runtime decision brief](worker-shutdown/renderer-decision-brief.md) compares the unverified existing-platform bootstrap with externally managed disposable job containers; design only, no redesign or infrastructure operation authorized.
