...
root=base/'worker-oci-bundle/rootfs'
root.mkdir()
with tarfile.open(base/'devotional-worker.oci.tar') as outer:
 def content(d):
 ...
 assert config['config']['Cmd']==['node','dist/server.js']
 assert config['config']['WorkingDir']=='/app/apps/shorts-worker'
 (base/'evidence/worker-image-config.json').write_text(json.dumps(config,indent=2)+'\n')
 (base/'evidence/worker-image-manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
 for number,d in enumerate(manifest['layers']):
  layer=base/'worker-layer.tar'
