...
expected='/user.slice/user-1000.slice/user@1000.service/app.slice/studio460-worker-image2'
...
result={'pid':pid,'expected':expected,'before':snap(),'ancestors':{},'observed':{},'violations':[]}
...
while time.monotonic()-start<80:
 ...
 if (b/'worker-monitor-stop').exists():break
 time.sleep(.05)
result['elapsedSeconds']=time.monotonic()-start;result['after']=snap()
(b/'evidence/worker-membership-bounded.json').write_text(json.dumps(result,indent=2)+'\n')
