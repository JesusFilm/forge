...
- A later fresh-server cold pass recorded one transient unready sample at frame 188: readyState 1, seeking true, paused false, videoWidth 320, source time 8.472607s. Replay reached frame 388 with zero unready samples and no errors. Another run while package validation was active sampled frame 105 unready. These are retained diagnostic failures of the strict readiness assertion; they do not establish visible black pixels during resynchronization. The fixture now records per-video state for such samples. The fix addresses missing incoming decoded media at cuts; do not claim immunity to every seek/buffering artifact under load.
...
- Manager and shorts-compositions typechecks passed. Touched TypeScript lint passed. CI subsequently caught a Manager-specific React ref-write rule; moving that update into an effect preserves seek ordering. Package-level Manager lint was rerun to cover its nested configuration. Manager production build passed with the documented mock data/backend configuration and a local-only mock session secret.
