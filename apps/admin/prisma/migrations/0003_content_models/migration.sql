-- Unit 4 — Real content types: Experience, Video, and Core-sourced reference
-- data. See docs/plans/2026-04-13-002-feat-admin-app-graphql-postgres-plan.md.
--
-- Notable design choices:
--  * Per-locale rows for Experience and Video content (independent publish
--    state per locale; per-locale embeddings later).
--  * JSON `name` for low-cardinality reference data (Language, Country, etc).
--  * Every Core-sourced row carries `core_id` (UNIQUE), `source` enum,
--    `core_updated_at`, `synced_at`. UNIQUE on `core_id` is the contract Core
--    sync upserts depend on (Unit 10).
--  * Experience.embedding HNSW index is PARTIAL (`WHERE embedding IS NOT
--    NULL`) so NULL rows are excluded by design and planner stats stay clean.
--    Built CONCURRENTLY so future re-indexes don't lock writes (split into a
--    separate migration step because CONCURRENT cannot run in a transaction).
...
CREATE TABLE "video_variant" (
    ...
    "created_at"             TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updated_at"             TIMESTAMPTZ  NOT NULL,
    CONSTRAINT "video_variant_video_id_fkey"
        FOREIGN KEY ("video_id")         REFERENCES "video"("id")         ON DELETE CASCADE,
    CONSTRAINT "video_variant_language_id_fkey"
        FOREIGN KEY ("language_id")      REFERENCES "language"("id")      ON DELETE SET NULL,
    CONSTRAINT "video_variant_video_edition_id_fkey"
        FOREIGN KEY ("video_edition_id") REFERENCES "video_edition"("id") ON DELETE SET NULL,
    CONSTRAINT "video_variant_mux_video_id_fkey"
        FOREIGN KEY ("mux_video_id")     REFERENCES "mux_video"("id")     ON DELETE SET NULL
);
CREATE INDEX "video_variant_video_id_idx"         ON "video_variant"("video_id");
CREATE INDEX "video_variant_language_id_idx"      ON "video_variant"("language_id");
CREATE INDEX "video_variant_video_edition_id_idx" ON "video_variant"("video_edition_id");
CREATE INDEX "video_variant_mux_video_id_idx"     ON "video_variant"("mux_video_id");
CREATE INDEX "video_variant_deleted_at_idx"       ON "video_variant"("deleted_at");

CREATE TABLE "video_variant_download" (
    "id"               TEXT        PRIMARY KEY,
    ...
    "height"           INTEGER,
    "created_at"       TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT "video_variant_download_video_variant_id_fkey"
        FOREIGN KEY ("video_variant_id") REFERENCES "video_variant"("id") ON DELETE CASCADE
);
CREATE INDEX "video_variant_download_video_variant_id_idx"
    ON "video_variant_download"("video_variant_id");
