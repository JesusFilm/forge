...
describe("readSceneAnalysisArtifact", () => {
  ...
  // wipes in its own afterEach when Vitest runs files concurrently).
  ...
  it("returns the parsed result for a well-formed artifact", async () => {
    readArtifactSpy.mockResolvedValueOnce(stubArtifactBytes(validArtifact))
    ...
    expect(readArtifactSpy).toHaveBeenCalledWith(
      "test-asset-valid",
      "scene-analysis",
      "json",
    )
  ...
  it("returns an empty scenes array without error when scenes is empty", async () => {
    readArtifactSpy.mockResolvedValueOnce(stubArtifactBytes({ scenes: [] }))
  ...
  it("throws artifact_invalid when the payload fails the schema", async () => {
    readArtifactSpy.mockResolvedValueOnce(
      stubArtifactBytes({ scenes: [{ sceneIndex: "nope" }] }),
    )
    const error = await readSceneAnalysisArtifact(
      "test-asset-wrong-shape",
    ).catch((caught) => caught)
