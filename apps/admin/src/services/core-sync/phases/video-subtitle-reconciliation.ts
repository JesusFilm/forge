...
export class SubtitleReconciliationError extends Error {
  constructor(
...
export class ResidualVideoError extends Error {
  constructor(
    readonly code: string,
    message: string,
  ) {
...
export function isSnapshotMismatch(error: unknown): boolean {
  return (
    error instanceof VideoSubtitleSnapshotMismatchError ||
...
export async function loadPreviousDiagnostic(
  prisma: PrismaClient,
...
export async function loadAdminProjection(
  prisma: PrismaClient,
...
export function mismatchedVideoIds(
  ...
  const [videos, languages] = await Promise.all([
    ...
    prisma.language.findMany({
      where: {
        deletedAt: null,
        source: "CORE",
        coreId: { in: requestedLanguageIds },
      },
      select: { id: true, coreId: true },
    }),
  ])
...
export function completedDiagnostic({
  checkId,
  startedAt,
  completedAt,
  core,
  ...
  return {
    ...
    initialMismatchVideoIds: sortedIds(initialMismatchVideoIds),
    repairedVideoIds: sortedIds(repairedVideoIds),
    residualVideoIds: sortedIds(residualVideoIds),
...
export function inSyncEvidence(
