...
import {
  ...
  ACTIVE_CONTENT_EMBEDDING_CONTRACT_SEED as coverageContract,
...
describe.skipIf(!RUN_REAL_DB_TEST)("coverage database stages", () => {
  const database = `coverage_test_${randomUUID().replaceAll("-", "")}`
  const input = {
    seedMediaId: "seed",
    locale: "te",
    audioLanguageSlug: "telugu",
  }
  ...
  beforeAll(async () => {
    ...
    if (!["localhost", "127.0.0.1", "[::1]"].includes(url.hostname)) {
      throw new Error("Coverage fixtures require an explicitly local database")
    }
    ...
    await fixture.query(
      ...
      [
        coverageContract.id,
        coverageContract.query.provider,
        coverageContract.query.model,
        coverageContract.query.nativeDimensions,
        coverageContract.query.dimensions,
        coverageContract.query.transformVersion,
        coverageContract.storage.provider,
        coverageContract.storage.model,
        coverageContract.storage.nativeDimensions,
        coverageContract.storage.dimensions,
        coverageContract.storage.transformVersion,
      ],
    )
    await fixture.query(
      "INSERT INTO content_embedding_contract_pointer VALUES ($1,$2)",
      [CONTENT_EMBEDDING_CONTRACT_POINTER_ID, coverageContract.id],
    )
  })
  ...
  it.each([
    ...
    ["DELETE FROM video_transcript WHERE id='seed'", "seed_transcript_missing"],
    [
    ...
    ["UPDATE video_locale SET deleted_at=now()", "published_metadata_missing"],
