...
function isMissingAvailabilityProjection(
  ...
  return (
    ...
    (error.status === 404 || error.message.includes(collection))
  )
}
...
function isMissingLexicalProjection(
  ...
  return (
    ...
    new RegExp(
      `${collection.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}|title_[a-z]|metadata_[a-z]|canonicalVideoId|languageIdentity`,
    ).test(error.message)
  )
}
...
export class TypesenseWatchSearchService {
  ...
  private async executeSearch(
    ...
    hydratedById = await this.withPublishedLocaleProjection(
      new Map(
        pageCandidates.flatMap(({ candidate }) => {
          const hydrated = hydratedById.get(candidate.videoId)
          return hydrated ? [[candidate.videoId, hydrated] as const] : []
        }),
      ),
      preferredLocale,
    )
  ...
  private async hydrateResultDocuments(
    ...
    try {
      const [catalogResult, ...availabilityResults] = await this.multiSearch<
  ...
  private async withPublishedLocaleProjection(
    ...
    const requestedLocales = [preferredLocale, broadLocale, "en"].filter(
    ...
    const localeRows = await this.prisma.videoLocale.findMany({
      where: {
        ...
        status: "PUBLISHED",
        deletedAt: null,
        OR: [{ locale: { in: requestedLocales } }, { languageSlug: "english" }],
      },
      orderBy: { id: "asc" },
      select: {
        videoId: true,
