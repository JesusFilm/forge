...
builder.queryFields((t) => ({
  ...
  managerJobsTotal: t.field({
    type: "Int",
    authScopes: { hasPermission: "write:manager-jobs" },
    resolve: (_root, _args, ctx) =>
      ctx.services.managerJob.count({ user: ctx.user }),
  }),
  managerJob: t.field({
    type: ManagerJobRef,
    nullable: true,
    ...
    resolve: async (_root, args, ctx) => {
      try {
        return await ctx.services.managerJob.get({
          user: ctx.user,
          id: String(args.id),
        })
      } catch (error) {
...
builder.mutationFields((t) => ({
  createManagerJob: t.field({
    ...
    args: {
      ...
      sourceLanguageId: t.arg.string({ required: false }),
      sourceLanguageCode: t.arg.string({ required: false }),
      sourceSelectionReason: t.arg.string({ required: false }),
      primaryRequestedTargetLanguageCode: t.arg.string({ required: false }),
      resolvedTargetLanguageCodes: t.arg.stringList({ required: false }),
      sourceCollectionTitle: t.arg.string({ required: false }),
      sourceMediaTitle: t.arg.string({ required: false }),
      requestedLanguageAbbreviations: t.arg.stringList({ required: false }),
    ...
    resolve: (_root, args, ctx) =>
      ctx.services.managerJob.create({
        ...
        input: {
          ...
          ...(args.sourceLanguageId != null
            ? { sourceLanguageId: args.sourceLanguageId }
            : {}),
          ...(args.sourceLanguageCode != null
            ? { sourceLanguageCode: args.sourceLanguageCode }
            : {}),
          ...(args.sourceSelectionReason != null
            ? { sourceSelectionReason: args.sourceSelectionReason }
            : {}),
          ...(args.primaryRequestedTargetLanguageCode != null
            ? {
                primaryRequestedTargetLanguageCode:
                  args.primaryRequestedTargetLanguageCode,
              }
            : {}),
          resolvedTargetLanguageCodes:
            args.resolvedTargetLanguageCodes?.filter(Boolean) ?? [],
          ...(args.sourceCollectionTitle != null
            ? { sourceCollectionTitle: args.sourceCollectionTitle }
            : {}),
          ...(args.sourceMediaTitle != null
            ? { sourceMediaTitle: args.sourceMediaTitle }
            : {}),
          requestedLanguageAbbreviations:
            args.requestedLanguageAbbreviations?.filter(Boolean) ?? [],
      ...
      ctx.services.managerJob.update({
        ...
        input: {
          ...
          ...(args.artifacts !== undefined
            ? { artifacts: args.artifacts }
            : {}),
          ...(args.sourceLanguageId !== undefined
            ? { sourceLanguageId: args.sourceLanguageId ?? undefined }
            : {}),
          ...(args.sourceLanguageCode !== undefined
            ? { sourceLanguageCode: args.sourceLanguageCode ?? undefined }
            : {}),
          ...(args.sourceSelectionReason !== undefined
            ? {
                sourceSelectionReason: args.sourceSelectionReason ?? undefined,
              }
            : {}),
          ...(args.primaryRequestedTargetLanguageCode !== undefined
            ? {
                primaryRequestedTargetLanguageCode:
                  args.primaryRequestedTargetLanguageCode ?? undefined,
              }
            : {}),
          ...(args.resolvedTargetLanguageCodes !== undefined
            ? {
                resolvedTargetLanguageCodes:
                  args.resolvedTargetLanguageCodes?.filter(Boolean) ?? [],
              }
            : {}),
          ...(args.sourceCollectionTitle !== undefined
            ? {
                sourceCollectionTitle: args.sourceCollectionTitle ?? undefined,
              }
            : {}),
          ...(args.sourceMediaTitle !== undefined
            ? { sourceMediaTitle: args.sourceMediaTitle ?? undefined }
            : {}),
          ...(args.requestedLanguageAbbreviations !== undefined
            ? {
                requestedLanguageAbbreviations:
                  args.requestedLanguageAbbreviations?.filter(Boolean) ?? [],
              }
