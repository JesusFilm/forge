...
// The workflow reads the gateway gate (AI_GATEWAY_CHAT_*) AND the U4
// trusted-flag gate (AI_GATEWAY_CONSTRAINED_DECODING_TRUSTED) at call
// time to decide whether each phase requests provider-native structured
// output. Default all three to the off state so every pre-existing test
// runs the plain text → parse-ladder path; constrained-decoding tests
// flip them. The trusted flag is the U4 precondition: even with the
// gateway enabled, structured opts are withheld until it is "true".
...
describe("multiStepDraftWorkflow (U4 — real step bodies)", () => {
  ...
  describe("structural invariants", () => {
    ...
    it("declares its max-step ceiling as 5 (plan → skeleton → fill → critique → revise)", () => {
      ...
      // would change this constant and trip the test. The U3 two-phase
      // rebuild replaced the single draft step with skeleton + fill.
  ...
  describe("structured output (gateway path)", () => {
    ...
    // Enable BOTH preconditions for actually sending structured opts:
    // the gateway provider gate AND the U4 trusted flag. Constrained
    // decoding is only sent to the provider when both are on.
  ...
  describe("WorkflowStepError discriminant", () => {
    ...
    it("accepts the new U3 step names (skeleton / fill)", () => {
  ...
  // -------------------------------------------------------------------------
  describe("two-phase generation (U3)", () => {
    const PLAN_INPUT = {
    ...
    // looked up by id "experience-fill"; we route per-call by inspecting
    // the prompt (which names the target type) so a single mock agent can
    // serve every node in document order.
    ...
    describe("executeSkeletonStep", () => {
      ...
      it("rejects a section-inside-section skeleton (AE1) and logs skeleton_validation_failed", async () => {
        const warnSpy = vi
          .spyOn(console, "warn")
    ...
    describe("executeFillStep", () => {
      ...
      it("fills SEQUENTIALLY in deterministic skeleton order; a later fill sees earlier filled blocks", async () => {
        ...
        // Capture, per fill call, whether the prompt carries the "(none
        // yet)" sentinel and whether it embeds the heading of an
        // already-filled block (the coherence accumulator threading
        ...
        const { mastra } = makeFillMastra({
          onFill: (prompt) => {
            ...
            // every LATER fill prompt must embed that prior block (its
            // heading) in the "blocks already written" context.
    ...
    describe("workflow chain symmetry", () => {
      it("both workflows commit and expose .draft-carrying output schemas", () => {
        ...
        // ({ ...planFields, draft }). Both carry `.draft` at the top level
        // (the action's cast relies on this).
        expect(multiStepDraftWorkflow.id).toBe("multi-step-draft")
        expect(quickDraftWorkflow.id).toBe("quick-draft")
  ...
  // -------------------------------------------------------------------------
  // U4 — per-phase schema-constrained decoding (gated + truncation guard).
  // Covers AE2: the final guarantee never depends on constrained decoding.
  // -------------------------------------------------------------------------
  describe("per-phase constrained decoding (U4)", () => {
    const PLAN_INPUT = {
      prompt: "A page about hope in difficult seasons",
      locale: "en",
      candidates: [] as never[],
    ...
    // A constrained-decoding-aware mock surface: every agent records the
    // opts it was called with so the test can assert whether structured
    // opts were threaded. The fill agent routes by the type named in the
    // prompt (same convention as the U3 makeFillMastra helper).
    ...
    describe("default path (trusted flag OFF) — free-text + coercion", () => {
      beforeEach(() => {
        // Gateway provider enabled but constrained decoding NOT trusted —
        // the U4 default. Even with the gateway on, no structured opts.
        mockEnv.env.AI_GATEWAY_CHAT_API_KEY = "sk-test"
        mockEnv.env.AI_GATEWAY_CHAT_ENABLED = "true"
        mockEnv.env.AI_GATEWAY_CONSTRAINED_DECODING_TRUSTED = "false"
      })
      ...
      it("fill step does NOT pass structuredOutput / toolChoice, yet still produces a valid draft via the free-text path", async () => {
        ...
        // And coercion + validation still produced a valid draft (AE2: the
        // guarantee does not depend on constrained decoding).
    ...
    describe("trusted path (trusted flag ON) — per-phase structured opts", () => {
      ...
      it("each fill call passes that node's flat block schema", async () => {
        ...
        // Fill order is videoHero, text (section child), cta — each gets its
        // own flat schema.
