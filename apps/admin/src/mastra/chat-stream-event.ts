...
export type ChatStreamEvent =
  ...
  | {
      ...
      messageId: string
      /**
       * Producer id (agent or workflow) that emitted this assistant
       * turn, surfaced from `ExperienceChatMessage.produced_by`. The
       * chat panel reads it to decide whether to render the 👍/👎
       * rating control (only for producers in
       * `RATABLE_PRODUCERS` — see
       * `apps/admin/src/services/chat-rating.constants.ts`).
       */
      producedBy: string
    }
