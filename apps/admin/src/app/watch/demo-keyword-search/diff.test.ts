...
import { buildProvenanceMap, computeTopKDiff } from "./diff"
...
describe("buildProvenanceMap", () => {
  it("records source membership per id within top-k", () => {
    const map = buildProvenanceMap(["a", "b"], ["a", "c"], 10)
    expect(Array.from(map.get("a") ?? [])).toEqual(["H", "K"])
    expect(Array.from(map.get("b") ?? [])).toEqual(["H"])
    expect(Array.from(map.get("c") ?? [])).toEqual(["K"])
  })

  it("respects k truncation per source", () => {
    const map = buildProvenanceMap(["a", "b"], ["b"], 1)
    // Only the first id of each source counts at k=1:
    // hybrid -> a, keyword -> b
    expect(Array.from(map.get("a") ?? [])).toEqual(["H"])
    expect(Array.from(map.get("b") ?? [])).toEqual(["K"])
  })

  it("returns an empty map for k <= 0", () => {
    expect(buildProvenanceMap(["a"], ["b"], 0).size).toBe(0)
  })
})
