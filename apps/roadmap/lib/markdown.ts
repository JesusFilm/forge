...
function formatDueDate(feature: Feature): string {
...
function renderReadmeFeatureTable(features: Feature[]): string[] {
  return [
...
export function renderRoadmapReadme(
  features: Feature[],
  today = new Date(),
): string {
  const counts = getStatusCounts(features)
  ...
  const overdueCount = countOverdueOpenFeatures(features, normalizedToday)
