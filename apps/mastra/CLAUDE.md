...
- Homepage storefront curation is Mastra-owned through the private, zero-tool
  `storefrontCuratorAgent` and `storefront-homepage-curation` workflow. The
  agent is imported by the workflow but deliberately absent from Mastra's agent
  registry: deterministic code supplies its bounded evidence and owns every
  Admin MCP call. The enabled-locale CSV defaults to `en`; mode defaults to
  `off`; and the Monday 06:00 UTC schedule is absent unless the independent
  `STOREFRONT_CURATOR_SCHEDULE_ENABLED` flag is true. Manual execution and
  bounded stored-run inspection use only the dedicated-bearer
  `/forge-storefront-curation` routes. All Mastra native workflow routes for
  this workflow are denied, and the dedicated bearer pool must be disjoint from
  `MASTRA_SERVICE_API_KEYS`. Check model-provider
  readiness before reading Admin. Admin context stays minimal and read-only: no
  effective draft body, preview token, or preview URL. Deterministic validation
  separates collection-parent evidence from leaf media checks, and replacement
  sections occupy the first prior `storefront-curator-` slot without moving
  human blocks. Staging sends a UUID operation ID plus a normalized block
  digest, never retries the write, and reconciles ambiguous responses only by
  exact Admin draft attribution; otherwise return `stage_outcome_unknown`.
  `writeOutcome`/`draftStaged`, not legacy `changed`, are the write receipt. The
  workflow never calls publish/discard. Operator procedure:
  `docs/runbooks/storefront-curator-agent.md`.
...
| `STOREFRONT_CURATOR_SERVICE_API_KEYS`        | Dedicated CSV bearer allowlist for storefront curation starts and bounded sanitized run inspection. Optional and fail-closed when unset; boot-asserted disjoint from `MASTRA_SERVICE_API_KEYS`. Native storefront workflow routes never accept either pool.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `STOREFRONT_CURATOR_MODE`                    | Storefront write gate: `off` (default), `dry_run`, or `stage`. `stage` can create a shared Admin draft but can never publish/discard.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `STOREFRONT_CURATOR_ENABLED_LOCALES`         | Explicit locale rollout CSV, default `en`. A locale outside this set exits before Admin evidence is read.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `STOREFRONT_CURATOR_SCHEDULE_ENABLED`        | Independent schedule gate, default `false`. Only exact `true` registers the Monday 06:00 UTC workflow schedule; changing it requires process restart.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
