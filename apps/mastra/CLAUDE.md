...
- **Retention:** `src/mastra/ai-chat-retention.ts` purges threads by rolling
  ...
  days for non-`user:` resources, 180 days for `user:*`. Boot sweep + daily
  timer, bounded deletes, postgres-backend-gated, count-only logging. The
  purge is the adversarial storage backstop (the ceiling only bounds a
  cooperative client).
