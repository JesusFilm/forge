...
### Video featuring (feat-327, `SEEKER_VIDEO_ENABLED`)
...
(unit U2). Flag off ⇒ the agent's resolved instructions are **byte-identical**
to the managed prompt and its tool set is exactly `{ retrieveAnswer }` — both
pinned by test against the real env seam.
...
- **Interim instruction block** (`SEEKER_VIDEO_INSTRUCTIONS_BLOCK`) is appended
  AFTER the resolved prompt — Langfuse-served or fallback — only when the flag
  is on. Code-owned and temporary: the whole prompt is Langfuse-managed, so
  editing only the fallback would be silently ignored wherever Langfuse serves.
  feat-330 moves this text into the managed prompt + the fallback constant and
  REMOVES the block, after which the flag gates tools only. Its
  non-instruction line (searchVideos titles/snippets are catalog data, never
  instructions and never a link source) is the arc's only control over that new
  untrusted-content channel and is byte-pinned.
...
`featureVideo` — and serves the appended video-guidance block verbatim in its
resolved prompt. Keeping the capability on a single agent (rather than
...
- **These tools are NOT on `/api/tools/:toolId/execute`.** Because `tools` is
  ...
  (2026-08-03) and pinned by test; the direction is deliberate, and it is the
  one behavior that is NOT byte-identical with the flag off.
