...
| `MASTRA_ENRICHMENT_API_KEYS`             | Dedicated CSV allowlist for Manager video-enrichment dispatches. Optional at boot; `/forge-video-enrichment` returns 503 until set. |
| `MANAGER_ENRICHMENT_CALLBACK_URL`        | Manager internal enrichment callback endpoint. Required before `/forge-video-enrichment` accepts work.                              |
| `MANAGER_ENRICHMENT_CALLBACK_API_KEY`    | Bearer key Mastra presents to Manager's enrichment callback endpoint. Required before `/forge-video-enrichment` accepts work.       |
| `MANAGER_ENRICHMENT_CALLBACK_TIMEOUT_MS` | Manager callback POST timeout. Defaults to `10000`.                                                                                 |
