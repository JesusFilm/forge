...
import {
  ...
  type NormalizationErrorLike,
} from "./repair-draft"

/**
 * Build a normalize-shaped error. The standalone repair module is decoupled
 * from admin's `ExperienceAiNormalizationError` class (R2: no admin imports) —
 * the contract is the structural `NormalizationErrorLike` shape, so the test
 * constructs that shape directly. The load-bearing branch is the exhaustive
 * `code`-union switch.
 */
function normErr(
  code: NormalizationErrorLike["code"],
  message = "boom",
  cause?: unknown,
): NormalizationErrorLike {
  return { code, message, cause }
}

/**
 * Assert a rejection is the REAL `RepairDraftError` class with the given reason
 * (not just a same-shaped object) — mocked-shape-vs-real-contract discipline:
 * deleting the typed class must fail this test.
 */
...
describe("classifyRepairability", () => {
  // One fixture PER normalization code where ONLY that branch matches, so a
  // deleted branch fails (mocked-shape-vs-real-contract discipline).
  it("INVALID_BLOCKS -> schema_violation (repair-eligible)", () => {
    const err = normErr("INVALID_BLOCKS")
  ...
  it("BELOW_MIN_BLOCKS -> schema_violation (repair-eligible)", () => {
    const err = normErr("BELOW_MIN_BLOCKS")
    expect(classifyRepairability(err)).toBe("schema_violation")
    expect(isRepairEligible(classifyRepairability(err))).toBe(true)
  })

  it("UNKNOWN_VIDEO_REF -> structurally_impossible (NOT repair-eligible)", () => {
    const err = normErr("UNKNOWN_VIDEO_REF")
  ...
  it("UNKNOWN_SECTION_REF -> structurally_impossible (NOT repair-eligible)", () => {
    const err = normErr("UNKNOWN_SECTION_REF")
    expect(classifyRepairability(err)).toBe("structurally_impossible")
    expect(isRepairEligible(classifyRepairability(err))).toBe(false)
  })

  it("DUPLICATE_SECTION_REF -> structurally_impossible (NOT repair-eligible)", () => {
    const err = normErr("DUPLICATE_SECTION_REF")
    expect(classifyRepairability(err)).toBe("structurally_impossible")
    expect(isRepairEligible(classifyRepairability(err))).toBe(false)
  })
})
...
describe("serializeNormalizationError", () => {
  ...
  it("includes concrete Zod issue paths when the error carries a ZodError cause", () => {
    ...
    const err = normErr("INVALID_BLOCKS", "boom", zodError)
    const out = serializeNormalizationError(err)
...
// Mock the Mastra surface so repairDraft's getAgentById(...).generate(...) hits
// a spy. The REAL parse/lift/coerce/validate path runs on the agent's output so
// the repair branch is load-bearing.
...
describe("repairDraft", () => {
  ...
  it("accepts a flat {title, metaDescription, blocks} reply (no envelope)", async () => {
    ...
    const repaired = await repairDraft({
      draft: VALID_DRAFT,
      candidates: CANDIDATES,
      error: normErr("BELOW_MIN_BLOCKS"),
      attempt: 1,
