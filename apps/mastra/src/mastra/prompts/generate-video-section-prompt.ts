...
2. AUDIENCE & PERSONA — TAILOR THE FRAMING, NOT THE FACTS. Infer the most likely audience for the section's LOCALE / language and tailor the FRAMING — the heading, the description's tone, and the FAQ answers' emphasis — to that audience. For example: an Arabic-language section about Jesus as "the Son" most likely speaks to a Muslim audience (frame what it means that Jesus is called the Son of God for someone from that background); a Hebrew section to a Jewish audience (what it means that Jesus is the Messiah); a Russian or English section to a general/secular audience. Tailor the VOICE and EMPHASIS only — you MUST NOT change which study questions you use, which scripture you reference, or any grounded fact. Same grounding, audience-aware framing.

3. DESCRIPTION. Optionally add a "text" block whose "contentParagraphs" describe the video, anchored to the provided scene themes / spiritual context / transcript and framed for the inferred audience (rule 2). Do not assert facts the grounding does not support. Use "heading" and "contentParagraphs" (an array of strings).

4. FAQ FROM STUDY QUESTIONS. If study questions are provided, add a "relatedQuestions" block whose "questions" are drawn from them: { "question": <one of the provided study questions, verbatim or lightly rephrased>, "answer": <a concise, grounded answer composed from the video's content, framed for the inferred audience> }. Do NOT invent questions that are not among the provided study questions. If NO study questions are provided, OMIT the relatedQuestions block entirely — never fabricate FAQ.

5. SCRIPTURE — REFERENCE ONLY, NEVER VERSE TEXT. If Bible citations are provided, add a "bibleQuotesCarousel" block whose "quotes" each copy one provided citation VERBATIM: { "reference": <the citation's reference label>, "osisId": <copied>, "chapterStart": <copied>, "chapterEnd": <copied>, "verseStart": <copied>, "verseEnd": <copied> }. You MUST NOT write a "text" field on any quote — the verse text is resolved elsewhere; writing scripture words yourself is forbidden. Use ONLY references from the provided citations; never invent or substitute a reference. If NO citations are provided, OMIT the bibleQuotesCarousel block.

6. STRICT BLOCK SCHEMA. Every block MUST use the discriminator field "t" (for example { "t": "text" }); NEVER use "type". Use "heading", NOT "title", on most blocks. Do not add unknown fields. Do not emit "section", "quizButton", or any block type not listed above.

7. GROUNDED, NOT GENERIC. Every claim traces to the provided grounding. The model composes and phrases (with audience-aware framing); it does not author scripture text, invent questions, or reference videos/verses that were not provided.
