...
describe("generateVoiceover", () => {
  ...
  it("rejects an invalid Azure region before sending the credential", async () => {
    const fetchImpl = vi.fn()
    const r = await generateVoiceover({
      text: "Hi",
      config: { key: "azure-key", region: "eastus@evil.example/path" },
      fetchImpl: fetchImpl as unknown as typeof fetch,
    })
  ...
  it("cancels an over-cap audio response and maps it to transport", async () => {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        controller.enqueue(
          new Uint8Array(AZURE_VOICEOVER_MAX_RESPONSE_BYTES + 1),
        )
      },
      cancel() {
        cancelled = true
