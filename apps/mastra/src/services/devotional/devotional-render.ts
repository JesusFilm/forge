...
async function arclightClipUrl(mediaId: string): Promise<string> {
  ...
  if (!url) throw new Error(`Arclight ${mediaId}: no downloadUrls`)
  return url
}
...
function trimClip(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      c.kill("SIGKILL")
      reject(new Error(`ffmpeg trim timed out after ${FFMPEG_TIMEOUT_MS}ms`))
    }, FFMPEG_TIMEOUT_MS)
    c.on("error", (e) => {
      clearTimeout(timer)
      reject(e)
    ...
    c.on("close", (code) => {
      ...
      if (code === 0) resolve()
        ...
        reject(new Error(`ffmpeg trim exit ${code}: ${err.trim().slice(-500)}`))
    })
  })
}
...
async function renderInStage(
  ...
  const COVER_LEAD_SEC = 2.2 // intro hold (~1s) + cover tail, added on-screen
  const bgTimelineSec = manifest.cards.reduce((sum, card) => {
    if (card.kind === "video") return sum
    return (
      sum +
      (Number(card.durationSec) || 3) +
      (Number(card.holdSec) || 0) +
      (card.kind === "cover" ? COVER_LEAD_SEC : 0) +
      0.4 // per-card tail (~CARD_TAIL_FRAMES)
    )
  }, MARGIN) // + trailing crossfade / safety
  ...
  const videoPath = path.join(
    options.outDir,
    `${slug}-seq${devo.sequence}${suffix}.mp4`,
  )
...
export async function prepareAndRenderDevotional(
  ...
  const cacheDir = cacheDirFor(input.chapterIndex, input.sequence)
