#!/usr/bin/env tsx
/**
 * PROTOTYPE — STEP 0. Capture real RAG results as fixtures.
 *
 * Runs every question against a live JesusFilm RAG and records exactly what
 * `retrieveAnswer` would have handed the model. The recording is committed, so
 * later runs are reproducible without the RAG being up — and, critically, so a
 * score change between two runs cannot be silently caused by the corpus moving
 * underneath them.
 *
 ...
 *     pnpm --filter @forge/mastra proto:capture-rag
 */
...
const DEFAULT_OUT = "src/prototypes/chat-eval/fixtures/rag-fixtures.json"
...
function corpusHash(fixtures: readonly RagFixture[]): string {
