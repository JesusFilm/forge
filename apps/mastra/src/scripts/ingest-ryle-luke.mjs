 ...
 *
 * Output: devo/corpus/ryle-luke.json — one entry per section (e.g.
 * "Luke 9:12-17"), each: { id, book, chapter, reference, osisRef, verse,
...
const CHAPTERS = 24
const OUT = path.join(REPO_ROOT, "devo/corpus/ryle-luke.json")
...
function win1252ToUtf8(buf) {
  return Buffer.from(buf).toString("latin1").replace(/[\x80-\x9f]/g, (ch) => {
    const CP1252 = {
      0x91: "‘",
      0x92: "’",
      0x93: "“",
...
function parseChapter(html, chapterNum) {
  ...
  for (let i = 0; i < anchors.length; i++) {
    ...
    entries.push({
      id: osisRef ?? `Luke_${chapterNum}_${sectionNum}`,
      book: "Luke",
      chapter: chapterNum,
...
async function main() {
  ...
  const corpus = {
    ...
    ingestedFrom: "gracegems.org (legacy HTML, windows-1252)",
