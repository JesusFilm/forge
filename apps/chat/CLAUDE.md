...
src/
  app/
    ...
    c/[id]/page.tsx      feat-209 deep-link entry: force-dynamic, mirrors page.tsx's resolution order (auth config → identity → seeker gate → signin marker) and maps the pure resolver onto AppShell (granted → adopted seed; else deniedScreen); noindex metadata, no thread titles in the head. The /c/<id> GET necessarily puts the thread id in Cloudflare/Railway HTTP access logs — accepted KTD9 residual, scoped claim in the route's docstring
    api/
  ...
  components/
    shell/
      app-shell.tsx      'use client' — owns conversation state (useConversations) + sidebar view state (collapsed rail / mobile drawer open); matchMedia breakpoint reset, body scroll-lock, <main> inert focus-trap; mobile-only top bar (menu trigger + brand, feat-270 — the drawer trigger never floats over transcript text); feat-209: mounts the URL-sync hook on granted shells, swaps <Chat> for the denial pane (deniedScreen, or the deep-link row's not_available escalation), and announces popstate-driven conversation changes via a polite live region
  ...
  lib/
    ...
    deep-link-entry.ts   feat-209: pure KTD5 entry resolver for /c/<id> — unavailable / sign_in / granted precedence (malformed id and unconfigured auth deny before the identity branch; gate-denied denies after it)
