...
// KTD8: the gate is a build-time constant in a dependency-free leaf, so it
// flips by OTA update alone. Three properties hold that shape, and no
// behavioural suite ties them together — a suite that injects the gate as a
...
const FEATURE = path.join(MOBILE, "src", "lib", "lapseReminders")
const FLAG = path.join(FEATURE, "constants.ts")
...
// the way rawExportConstants.ts does, so a consumer's off branch is live code.
const FLAG_PATTERN =
  /^export const LAPSE_REMINDERS_ENABLED(?:: boolean)? = (true|false)$/gm
...
const ANY_DECLARATION =
  /(?:^|\s)(?:export\s+)?const\s+LAPSE_REMINDERS_ENABLED\b/
...
describe("the lapse reminders kill-switch", () => {
  it("parses the flag the way a reader must (positive control)", () => {
    expect(readFlags("export const LAPSE_REMINDERS_ENABLED = false")).toEqual([
      false,
    ])
    expect(
      readFlags("export const LAPSE_REMINDERS_ENABLED: boolean = true"),
    ).toEqual([true])
    // A rewrite that breaks the one-line contract must fail, not default.
    expect(
      readFlags("export const LAPSE_REMINDERS_ENABLED = !__DEV__"),
    ).toEqual([])
    expect(readFlags("const LAPSE_REMINDERS_ENABLED = false")).toEqual([])
    expect(
      readFlags("export const LAPSE_REMINDERS_ENABLED =\n  false"),
    ).toEqual([])
    // A duplicate inside a block comment is what a first-match read would take.
    expect(
      readFlags(
        "/* export const LAPSE_REMINDERS_ENABLED = true */\nexport const LAPSE_REMINDERS_ENABLED = false",
      ),
    ).toEqual([false])
    expect(
      readFlags(
  ...
  it("keeps the constants file a leaf that imports nothing", () => {
    // The whole feature imports this file, including the gate check the tap
    // handler runs before the tree mounts. One import here can reach a native
    // module from every one of those call sites.
  ...
  it("finds a declaration wherever it sits (positive control)", () => {
    expect(
      ANY_DECLARATION.test("export const LAPSE_REMINDERS_ENABLED = true"),
    ).toBe(true)
    expect(ANY_DECLARATION.test("  const LAPSE_REMINDERS_ENABLED = true")).toBe(
      true,
    )
    expect(
      ANY_DECLARATION.test("const LAPSE_REMINDERS_ENABLED_TEMPORARILY = true"),
    ).toBe(false)
    expect(
      ANY_DECLARATION.test("import { LAPSE_REMINDERS_ENABLED } from './c'"),
    ).toBe(false)
  })
})
