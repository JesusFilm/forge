...
const agenticDryRunRequestSchema = z
  .object({
...
function isValidAgenticServiceRequest(request: Request): boolean {
...
function buildFailedResult(error: unknown): AutomationRunResult {
  const message =
    error instanceof Error
      ? error.message
      : "Agentic automation dry run failed."
  return {
    status: "failed",
    eligibleCount: 0,
    enqueuedCount: 0,
    ...
    jobDocumentIds: [],
    errors: [message],
    summary: "Agentic automation dry run failed.",
  }
}
...
async function completeDryRunOrMarkFailed(input: {
  ...
  try {
    ...
    try {
      ...
      console.error(
        "[api/automations/agentic-dry-run] Failed to read dry run before failure fallback:",
        readError,
      )
    }
    ...
    try {
      ...
      console.error(
        "[api/automations/agentic-dry-run] Failed to mark stranded dry run as failed:",
        fallbackError,
      )
    }
    throw error
...
async function releaseDryRunClaim(input: {
  ...
  try {
    ...
    console.error(
      "[api/automations/agentic-dry-run] Failed to release dry-run lease:",
      error,
    )
  }
}
...
export async function POST(
  ...
  const parsed = agenticDryRunRequestSchema.safeParse(body)
  if (!parsed.success) {
    return jsonError(
      "invalid_automation",
      "Agentic automation dry-run request is invalid.",
      400,
    )
  }
  if (parsed.data.runMode === "live") {
    return jsonError(
      "invalid_automation",
      "Agentic automation requests are dry-run only.",
      400,
    )
  }
  ...
  try {
    ...
    if (!isCreatableAutomationTemplate(automation.template)) {
      return jsonError(
        "invalid_automation",
        "Automation template is not available for Agentic dry runs.",
        400,
      )
    }
      ...
      return NextResponse.json(
        {
          ok: false,
          code: "dry_run_failed",
          message: "Agentic automation dry run failed.",
          data: {
            automationDocumentId: automation.documentId,
            run: completedRun,
          },
    ...
    console.error(
      "[api/automations/agentic-dry-run] Failed to run automation:",
      error,
    )
    return jsonError(
      "dry_run_failed",
      "Failed to run Agentic automation dry run.",
      502,
    )
  } finally {
    if (dryRunClaim) {
