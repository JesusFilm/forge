...
def verify_publication_approval(digest, policy, environment, reviews, actor_ids):
    """Validate GitHub-authenticated responses, never candidate-supplied records.

    The caller must obtain these responses from the fixed GitHub API for the
    current workflow run. No undocumented attempt/timestamp ordering is assumed.
    A conflicting rejection for these exact bytes requires a new candidate.
    """
    if (not matches(digest, '[a-f0-9]{64}') or not isinstance(actor_ids, list) or not 1 <= len(actor_ids) <= 2
            or any(type(actor) is not int or actor <= 0 for actor in actor_ids)):
        raise ReleaseRefused('Approval binding refused')
    reviewer_ids = verify_environment(policy, environment)
    if not isinstance(reviews, list) or len(reviews) >= 1000:
        raise ReleaseRefused('Bounded GitHub review history required')
    accepted = False
    for review in reviews:
        if (not isinstance(review, dict) or review.get('state') not in ('approved', 'rejected')
                or not isinstance(review.get('comment'), str)
                or not isinstance(review.get('user'), dict)
                or type(review['user'].get('id')) is not int
                or not isinstance(review.get('environments'), list)
                or any(not isinstance(item, dict) or type(item.get('id')) is not int
                       for item in review['environments'])):
            raise ReleaseRefused('Ambiguous GitHub review refused')
        bound = (review['comment'] == 'studio-candidate-sha256:' + digest
                 and any(item['id'] == policy['environmentId'] for item in review['environments']))
        if not bound:
            continue
        if review['state'] == 'rejected':
            raise ReleaseRefused('Candidate has a conflicting rejection')
        if review['user']['id'] in reviewer_ids and review['user']['id'] not in actor_ids:
            accepted = True
    if not accepted:
        raise ReleaseRefused('Actual reviewer approval for exact candidate required')


def verify_environment(policy, environment):
    exact_keys(policy, ['environmentId', 'reviewerIds'])
    reviewer_ids = policy['reviewerIds']
    if (type(policy['environmentId']) is not int or policy['environmentId'] <= 0
            or not isinstance(reviewer_ids, list) or not 1 <= len(reviewer_ids) <= 6
            or any(type(value) is not int or value <= 0 for value in reviewer_ids)
            or len(set(reviewer_ids)) != len(reviewer_ids)):
        raise ReleaseRefused('Configured approval identities required')
    if (not isinstance(environment, dict) or type(environment.get('id')) is not int
            or environment['id'] != policy['environmentId'] or environment.get('name') != 'studio-release'
            or not isinstance(environment.get('protection_rules'), list)):
        raise ReleaseRefused('Existing configured environment required')
    ...
    if len(rules) != 1 or rules[0].get('prevent_self_review') is not True:
        raise ReleaseRefused('Protected non-self review required')
    members = rules[0].get('reviewers')
    if not isinstance(members, list) or len(members) != len(reviewer_ids):
        raise ReleaseRefused('Environment reviewers differ from configured identities')
    observed = []
    for member in members:
        if (not isinstance(member, dict) or member.get('type') != 'User'
                or not isinstance(member.get('reviewer'), dict)
                or type(member['reviewer'].get('id')) is not int):
            raise ReleaseRefused('Explicit reviewer user identities required')
        observed.append(member['reviewer']['id'])
    if sorted(observed) != sorted(reviewer_ids):
        raise ReleaseRefused('Environment reviewers differ from configured identities')
    return reviewer_ids
