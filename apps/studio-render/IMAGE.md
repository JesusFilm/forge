...
The resumed local attempt uses the newly installed privileged mapping helpers and a full subordinate UID/GID range. The historical failure above remains valid for its earlier environment; the new devpts gid5 probe succeeds. Configured build-driver scope limits are 2CPU, 2GiB, no swap and 512 tasks; actual OCI executor bounds were not verified; production execution still requires 128 tasks. BuildKit and runc runtime sockets/state stay in task-owned rootfs paths. Exact-image results are recorded separately from the historical native harness and future deployed-container acceptance.
