...
- ISR cache: static watch routes under `src/app/[locale]/[htmlLang]/**` use route-level `revalidate = 3600`. Watch resolver `unstable_cache` wrappers in `src/lib/content.ts` / `src/lib/watch-home.ts` use the same `3600` second render TTL via `WATCH_RENDER_CACHE_REVALIDATE_SECONDS` and attach coarse tags from `src/lib/watch-cache-tags.ts`. The route response cache takes the lowest revalidate reachable during render, so lowering resolver cache TTLs also lowers the route `s-maxage`. `/api/revalidate` must invalidate both layers: `revalidatePath` for route output and `revalidateTag(tag, { expire: 0 })` for resolver data so webhook-triggered renders do not serve stale Data Cache first. The 1 hour route TTL is the fallback for a missed webhook or process-local invalidation miss.
...
Production proof on 2026-06-10 showed `@forge/web` online in Railway US West behind Cloudflare, live watch HTML served with `cf-cache-status: DYNAMIC`, and authorized `experience`, broad `video`, and `watch-route-manifest` webhooks returning healthy first post-webhook renders. A post-merge production check showed route exports alone did not change `s-maxage` while resolver caches still had `revalidate: 60`; the route-render resolver caches now match the 3600 second route TTL. The Railway CLI path available here did not expose exact web replica count.
