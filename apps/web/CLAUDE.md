...
The error sentinels are the deliberate exception: `/404` and the unavailable-language route render copy from the `locale` catalog, so `proxy.ts`'s `chromeDocumentLang` keeps a same-language regional refinement (`es-419` on Spanish copy) but drops a different content language (`aiw` on English copy). Keep `WATCH_ORDINARY_NOT_FOUND_INTERNAL_PATHS` built through that same helper — it must mirror exactly what `buildNotFound` emits.
