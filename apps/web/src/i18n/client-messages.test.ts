...
describe("route-scoped client messages", () => {
  ...
  it("keeps the end-of-video reflection translated on watch pages", () => {
    const messages = pickClientMessages(
      englishMessages,
      WATCH_CONTENT_CLIENT_MESSAGE_NAMESPACES,
    )
    ...
    expect(messages.WatchEndReflection?.dismiss).toBe("Close reflection")
    expect(messages.WatchEndReflection?.replay).toBe("Watch again")
  })
})
