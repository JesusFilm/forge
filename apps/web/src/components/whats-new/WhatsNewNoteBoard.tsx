...
export function WhatsNewNoteBoard({ contentClass }: { contentClass: string }) {
  ...
  function onTabKeyDown(event: ReactKeyboardEvent<HTMLButtonElement>): void {
    ...
    //
    // Feature-checked the way `SearchOverlay` does for the same call: jsdom
    // does not implement it, so an unguarded call throws out of the event
    // handler and surfaces as an unhandled error that fails the whole run
