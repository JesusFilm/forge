...
export function HeroPlayer({
  ...
  onReflectionDownload?: () => boolean | Promise<boolean>
  ...
  useEffect(() => {
    if (!chromeRevealed) return
    void import("@/components/watch/WatchEndReflection").catch(() => {
      // This import only prewarms the chunk. The dynamic component owns the
      // real load attempt when reflection is rendered, so a failed prewarm
      // must not become an unhandled rejection or disable the end state.
    })
  }, [chromeRevealed])
  ...
  const replayFromReflection = useCallback(() => {
    ...
    setWatchNextModeState(null)
    if (!currentPlayer) {
      dismissEndReflection()
      return
    }
    currentPlayer.currentTime = 0
    let result: void | Promise<void>
    try {
      result = currentPlayer.play()
    } catch {
      return
    }
    if (result && typeof (result as Promise<void>).then === "function") {
      void (result as Promise<void>).then(dismissEndReflection, () =>
        currentPlayer.pause(),
      )
      return
    }
  ...
  const openReflectionDownload = useCallback(() => {
    const result = onReflectionDownload?.()
    if (result && typeof (result as Promise<boolean>).then === "function") {
      void (result as Promise<boolean>).then(
        (opened) => {
          if (opened) setEndReflectionPlaybackIdentity(null)
        },
        () => {
          // Keep reflection open so the viewer can retry or choose another
          // action when download-session resolution fails unexpectedly.
        },
      )
      return
    }
    if (result !== false) setEndReflectionPlaybackIdentity(null)
  }, [onReflectionDownload])
