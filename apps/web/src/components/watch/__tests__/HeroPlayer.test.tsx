...
describe("HeroPlayer — initial mount", () => {
      ...
      root.render(
        <HeroPlayer
          block={makeBlock({
            ...
            subtitleLanguageCountLabel: "1 ulwimi",
          })}
  ...
  it("opens the subtitle picker once when subtitles are offered for one audio language", async () => {
    ...
    act(() => {
      root.render(
        <HeroPlayer
          block={makeBlock({ subtitleLanguageCountLabel: "1 language" })}
          onLanguageClick={onLanguageClick}
    ...
    expect(subtitleLanguageTag.tagName).toBe("BUTTON")
    expect(subtitleLanguageTag.textContent).toBe("Subtitles: 1 language")
    expect(subtitleLanguageTag.getAttribute("aria-label")).toBe(
  ...
  it("keeps a one-language count informational and omits unavailable factual tags", () => {
    ...
    act(() => {
      root.render(
        <HeroPlayer
          block={makeBlock({ audioLanguageCountLabel: "1 language" })}
          onLanguageClick={onLanguageClick}
          playableLanguageCount={1}
        />,
      )
  ...
  it("keeps mobile portrait header clearance while the preview contracts into playback", async () => {
    act(() => {
      root.render(<HeroPlayer block={makeBlock()} />)
    })
    ...
    expect(wrapper.getAttribute("data-mobile-portrait-header-clearance")).toBe(
      "true",
    )
  ...
  it("keeps custom overlay consumers on the existing muted preview frame", async () => {
    ...
    try {
      ...
      expect(
        wrapper.getAttribute("data-mobile-portrait-header-clearance"),
      ).toBe("false")
      expect(
        container.querySelector(
          '[data-testid="hero-player-mobile-header-band"]',
