...
describe("LanguageGlobeSection", () => {
  it("composes configurable copy and actions above one embedded globe", () => {
    ...
    expect(markup).toContain("leading-[1.08]")
    expect(markup).not.toContain("leading-[0.98]")
    expect(markup).toContain("pb-[calc(env(safe-area-inset-bottom,0px)+6px)]")
  ...
  it("supports a not-found h1 and decorative watermark", () => {
    ...
    expect(markup).toContain("leading-[0.98]")
    expect(markup).not.toContain("leading-[1.08]")
    expect(markup).toContain("pb-[calc(env(safe-area-inset-bottom,0px)+6px)]")
