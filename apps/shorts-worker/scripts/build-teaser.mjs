...
const cover = (d, f) => ({ kind: "cover", durationSec: d, bgFile: "clip.mp4", ...f })
const verse = (d, f) => ({ kind: "scripture", durationSec: d, bgFile: "clip.mp4", ...f })
...
async function main() {
  const manifestPath = abs(arg("manifest", "devo/artifacts/refuge/manifest.json"))
  ...
  const src = { cover: find("cover"), scripture: find("scripture"), video: find("video") }
  if (!src.video.videoFile) throw new Error("source devo has no video card / clip")
  if (!src.scripture.verse) throw new Error("source devo has no scripture verse")
  ...
  for (const [name, cards] of Object.entries(all)) {
    ...
    const total =
      cards.reduce((s, x) => s + x.durationSec + 0.4, 0) + 0.8 + 2
    console.log(`  ✓ ${name}  (${cards.length} cards, ~${total.toFixed(1)}s)  ${path.relative(REPO_ROOT, dir)}`)
  }
