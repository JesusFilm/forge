...
- Railway logsV2 silences info-level stdout from Next.js App Router runtime route handlers (Next.js 16 + Node 24 + standalone + logsV2:true). `console.log` from inside `apps/admin/src/app/api/*/route.ts` and GraphQL resolvers does NOT surface in Railway's deploymentLogs for runtime requests — even though `console.error`/`console.warn` do, and boot-time stdout does. **Default rule:** use `console.warn(JSON.stringify(...))` for structured per-request logging in admin's request path. See `docs/solutions/runtime-errors/railway-logsv2-silences-nextjs-stdout-runtime-20260518.md`.
