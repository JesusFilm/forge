...
```
apps/admin (Pothos GraphQL) -> packages/graphql adminGraphql() -> apps/web
apps/cms   (Strapi v5)      -> packages/graphql graphql()      -> apps/mobile
```
...
- `packages/graphql/` — gql.tada typed GraphQL client generated from Strapi and admin schemas during the migration window
...
- This package provides typed `graphql()` (Strapi) and `adminGraphql()` (admin) functions with gql.tada introspection types.
- After any Strapi or admin GraphQL schema change: run the matching schema emit and codegen steps.
- `apps/web` uses `adminGraphql()` against admin's public Pothos API. `apps/mobile` remains on Strapi via `graphql()` until its separate cutover.
